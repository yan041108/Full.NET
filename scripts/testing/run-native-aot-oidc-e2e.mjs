#!/usr/bin/env node
/**
 * 运行 Native AOT OIDC 外部进程 E2E（需先完成 linux-x64 publish）。
 */
...
const nativeGate = matrix.nativeAotOidcIntegration;
...
if (process.platform !== 'linux') {
  ...
  if (discoveredTests < nativeGate.minimum) {
    console.error(
      `Native AOT OIDC E2E 发现数不足：${discoveredTests} < ${nativeGate.minimum}。`
    );
    process.exit(1);
  }
  console.log(
    `Native AOT OIDC E2E 非 Linux 发现门禁：${discoveredTests} 项。`
  );
}
