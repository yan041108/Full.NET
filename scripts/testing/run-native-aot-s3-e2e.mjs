#!/usr/bin/env node
/**
 * 运行 Native AOT S3 Provider 外部进程 E2E（需先完成 linux-x64 publish）。
 */
...
if (process.platform !== 'linux') {
  ...
  if (discoveredTests < nativeGate.minimum) {
    console.error(
      `Native AOT S3 E2E 发现数不足：${discoveredTests} < ${nativeGate.minimum}。`
    );
    process.exit(1);
  }
  console.log(`Native AOT S3 E2E 非 Linux 发现门禁：${discoveredTests} 项。`);
}
