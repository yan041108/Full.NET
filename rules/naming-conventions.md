...
- 模块默认只创建 `Full.NET.Modules.{Module}` 主项目，并在其中承载模块实现、持久化、注册与 Endpoint；`Full.NET.Modules.{Module}.Contracts` 仅在存在真实跨模块或外部编译期消费者且需要稳定契约程序集隔离时创建，`Full.NET.Modules.{Module}.Http` 仅在同一 web-free Core 被非 HTTP 宿主真实复用并能证明独立传输适配收益时创建。可选项目沿用同一根 Namespace；一旦拆出 `.Http`，对应 Core 必须保持 web-free。完整门禁以 [`development-quality.md`](development-quality.md) 第 3 节为准；
