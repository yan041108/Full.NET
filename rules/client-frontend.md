...
9. **列表卡片内表格与分页布局**：在 `ElCard`（或等价列表壳）中展示 `el-table` 且需要占满可视区域时，表格区域必须在中间自适应高度，分页必须固定在卡片内容区底部，禁止把分页放在表格滚动区外单独堆叠导致整页滚动、也禁止仅用 `margin`/`padding` 手工顶分页而不绑定表格高度。
   - **适用**：单卡列表、左右分栏中的列表卡（`art-split-layout`）、带筛选/页签但列表主体仍是表格的页面。整页只有少量行、无高度撑满需求的只读块可豁免，但新增 CRUD/运维列表默认遵守本规则。
   - **有服务端或客户端分页**：必须使用 `useArtPagedTableInCard`（`ui/admin/src/framework/art-design/composables/useArtPagedTableInCard.ts`），列表卡加 `art-table-card`；表与 `ElPagination` 同包在 `ref="tableMainRef"` 的 `div.art-crud-table-main` 内；表格绑定 `:height="tableHeight"`；分页加 `class="art-table-pagination"`（全局样式 `margin-top: auto` 贴底）。列表 `loading` 结束后在 `finally` 中调用 `void syncTableLayout()`，以便行数变化后重新测量高度。
   - **无分页、仅表格撑满**：使用 `useArtCrudTableLayout`（同目录 `useArtCrudTableLayout.ts`），`bottomOffset` 取较小值（例如 `8`），同样使用 `art-crud-table-main` + `:height="tableHeight"`，并在数据加载 `finally` 中 `void nextTick(updateTableHeight)`。
   - **父级高度链**：页面根使用 `art-page-stack art-full-height`；分栏容器使用 `art-split-layout`；列表卡 `body` 必须为纵向 `flex` 且 `min-height: 0`，使 `art-crud-table-main` 获得 `flex: 1`。筛选、页签、提示条放在 `art-crud-table-main` **之外**（与表格同卡时由 `measureTableChromeHeight` 自动扣除高度）；不得把筛选条放进 `art-crud-table-main` 却又期望分页贴卡片底而不复用上述 composable。
   - **禁止**：在 scoped 样式中为分页单独写与 `art-layout.css` 重复的 `__pagination` 定位；用未设 `height` 的 `el-table` 依赖页面自然高度撑开；分页与表格分属不同卡片导致列表区无法一体伸缩。
   - **参考实现**：`ui/admin/src/views/LdapConnectionsView.vue`、`HostMessagingOpsView.vue`、`NotificationBindingsView.vue`；全局样式 `ui/admin/src/framework/art-design/theme/art-layout.css`（`.art-crud-table-main`、`.art-table-pagination`、`.art-split-layout`）。

验证：`pnpm --filter @fullnet/admin test`、Vue 生产构建、受影响 Vue E2E、`pnpm audit:clients`、`pnpm licenses list --prod --json`；列表表格布局变更须通过对应 View 单测，并在真实栈（若已纳入门禁）确认表体在卡片内滚动、分页贴列表卡底。设计背景见 [`client-ui-framework-design`](../docs/superpowers/specs/2026-07-18-client-ui-framework-design.md)与 [`vue-art-design-pro-adoption`](../docs/superpowers/plans/2026-07-18-vue-art-design-pro-adoption.md)。
