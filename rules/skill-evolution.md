...
| `fullnet-release-verification` | 自动化优先 / 7 | Release 构建、四套 MTP 测试、客户端单元/构建/E2E、许可证和漏洞审计再次用于双管理端国际化与可访问性交付；本机 Docker 管道差异被如实记录 | 继续收敛为跨平台验证脚本/CI，不优先创建判断型 Skill |
...
| `fullnet-seed-data-delivery` | 等待真实实现 / 0 | 已形成 System Bootstrap、Development/Demo Contributor、双库锁/审计和 Test Fixture 隔离设计，当前仍是 Migrator 硬编码 `--seed-local` | S0-S2 落地且第二个真实业务模块贡献双库幂等 Seed 后评估 |
