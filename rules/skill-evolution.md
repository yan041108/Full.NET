...
| `fullnet-release-verification` | 自动化优先 / 8 | Release 构建、四套 MTP 测试、客户端单元/构建/E2E、许可证和漏洞审计再次用于双管理端组件语言、HTTP 协商与账号偏好交付；本机 Docker 管道差异被如实记录 | 继续收敛为跨平台验证脚本/CI，不优先创建判断型 Skill |
...
| `fullnet-localization-delivery` | 候选 / 4 | L0 语言目录、L1 请求/错误本地化、账号偏好/租户默认语言双库持久化与 L2 双管理端组件语言、HTTP 协商和偏好同步已落地；双端验证了首渲染顺序、竞态、保存失败和稳定机器契约 | L2 落地首个双库可翻译业务数据，或下一个独立业务模块交付本地化资源时评估升级 |
