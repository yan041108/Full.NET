...
| 载体 | 解决的问题 | 示例 |
| --- | --- | --- |
| `AGENTS.md` / `rules/` | 核心底线与按任务适用的约束 | 中文注释、租户隔离、双数据库验证 |
...
没有新证据时不读取或修改候选计数，也不要求输出“未触发”状态。解除冻结需要项目所有者明确决定，或里程碑审计证明现有两个 Skill 无法覆盖已重复验证的工作流。
...
5. 独立能力变更逐项验证；同一权威规则的引用同步可同时更新受影响 Skill，并分别校验。工作区验证不要求先提交，提交和推送仍服从当前任务授权。
...
6. 实际修改或调整候选时在最终交付披露；无变化不输出例行状态。
...
| `fullnet-outbox-event-delivery` | 候选 / 1 | TenantProvisioned 是符合 ADR-0005 准入的重要业务事件；TenantChanged 只服务缓存失效，已从可复用 Outbox 证据中排除并登记为待迁移实现 | 第二个业务模块交付符合准入的重要可靠事件时升级，验证跨模块复用后的输入、重试和停止条件 |
...
| `fullnet-cache-feature` | 候选 / 3 | FusionCache 双抽象、租户 ID/域名 key 与 tag 失效已落地；ADR-0005 已冻结 C0/S0-L2/S1/S2/N0 与“直接 L1/L2 + Backplane + TTL/版本/权威源”，现有缓存失效 Outbox 属待迁移技术债而非目标证据 | 完成首个模块移除缓存 Outbox并通过多实例失败注入后，再结合第二种缓存模型按测试先行评估升级 |
...
| `fullnet-realtime-feature` | 候选 / 3 | SignalR JSON Hub、Notifications 提交后尽力推送、两个真实 API 宿主与专用 Redis 故障恢复，以及 Vue/Layui 认证连接、切上下文重连、通知刷新与失败降级均有自动化证据 | 第二个独立业务模块消费实时发布，或生产多副本编排与真实浏览器断网恢复形成完整生产流程后，再按测试先行评估升级；当前创建新 Skill 会扩大本任务范围 |
