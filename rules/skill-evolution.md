...
| `fullnet-api-compatibility` | 自动化优先 / 5 | ProblemDetails、Admin.NET Mapper、本地化认证 Challenge/429 已有真实 API 测试；本轮进一步以冻结夹具比较器和 PR base SHA CI 阻止 v1 路径、操作、安全字段与 schema 被静默破坏 | 等首个多客户端生成或真实 SDK 消费者落地后，再评估剩余人工决策是否足以形成 Skill；机械兼容检查继续保留在脚本、测试与 CI |
