...
test('Host 管理员可从真实 API 加载任务定义列表', async ({ page }, testInfo) => {
  test.skip(
    testInfo.project.metadata.clientKind === 'layui',
    '任务定义动作权限切片仅验证 Vue 管理端'
  );
...
test('受限 Host 账号访问任务定义 API 被拒绝且导航裁剪', async ({
  ...
  const response = await request.get(
    `${apiBaseUrl}/api/v1/jobs/host-definitions?page=1&pageSize=20`,
    {
      headers: {
  ...
  await expect(navigation.getByRole('link', { name: /任务调度/ })).toHaveCount(0);

  await page.goto(statusPath(clientKind, 'jobs/host-definitions'));
