...
test('Host 管理员可从真栈 API 加载文档分类列表', async ({ page }, testInfo) => {
  test.skip(
    testInfo.project.metadata.clientKind === 'layui',
    '文档分类动作权限切片仅验证 Vue 管理端'
  );
...
test('受限 Host 账号访问文档分类 API 被拒绝且导航裁剪', async ({
  page,
  request
}, testInfo) => {
  test.skip(
  ...
  const response = await request.get(`${apiBaseUrl}/api/v1/document/host/categories`, {
    headers: {
  ...
  await expect(navigation.getByRole('link', { name: /文档分类/ })).toHaveCount(0);

  await page.goto(statusPath(clientKind, 'document/categories'));
