...
test('受限 Host 账号访问超级管理员 API 返回 authorization.permission_denied', async ({
  page,
  request
}, testInfo) => {
  ...
  await expect(navigation.getByRole('link', { name: /超级管理员/ })).toHaveCount(0);

  await page.goto(statusPath(clientKind, 'identity/super-administrators'));
  await expect(page.getByText('403', { exact: true })).toBeVisible();
  await expect(page.getByRole('heading', { name: '没有访问权限' })).toBeVisible();
});

test('仅页面读权限用户调用用户写 API 返回 authorization.permission_denied', async ({
  request
}, testInfo) => {
  const clientKind = testInfo.project.metadata.clientKind;
  const origin = adminOrigin(clientKind);
  ...
  const response = await request.post(`${apiBaseUrl}/api/v1/identity/users`, {
    data: {
      username: `denied_${Date.now().toString(36)}`,
      displayName: '拒绝创建',
