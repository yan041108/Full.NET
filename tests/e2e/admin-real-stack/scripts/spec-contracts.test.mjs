...
test('oidc-center 真实栈必须覆盖 §6 最小消费探针', async () => {
  ...
  assert.match(fixturesSource, /expectOidcCenterAgentRunResumeRejected/u);
  assert.match(fixturesSource, /expectOidcCenterAgentRunCancelRejected/u);
  assert.match(source, /access token 可取消排队 Agent Run/u);
  assert.match(source, /access token 无法恢复排队 Agent Run/u);
  assert.match(source, /access token 无法重复取消 Agent Run/u);
  assert.match(source, /加载排队运行后不展示恢复按钮/u);
  assert.match(source, /切租户后加载排队运行不展示恢复按钮/u);
  assert.match(source, /切租户并返回 Host 后加载排队运行不展示恢复按钮/u);
  assert.match(source, /切租户后 access token 无法重复取消 Agent Run/u);
  assert.match(source, /切租户并返回 Host 后 access token 无法重复取消 Agent Run/u);
  assert.match(source, /取消后不再展示取消或恢复按钮/u);
  assert.match(source, /可通过 Agent 运行页 UI 加载已取消运行/u);
  assert.match(source, /切租户后可通过 Agent 运行页 UI 加载已取消运行/u);
  assert.match(source, /切租户并返回 Host 后可通过 Agent 运行页 UI 加载已取消运行/u);
  assert.match(source, /在 Agent 运行页加载运行后退出无法直达 Agent 运行页/u);
  assert.match(source, /在 Agent 运行页加载运行后强制下线无法直达 Agent 运行页/u);
  assert.match(source, /切租户后在 Agent 运行页加载运行后退出无法直达 Agent 运行页/u);
  assert.match(source, /切租户后在 Agent 运行页加载运行后强制下线无法直达 Agent 运行页/u);
  assert.match(source, /切租户并返回 Host 后在 Agent 运行页加载运行后退出无法直达 Agent 运行页/u);
  assert.match(source, /切租户并返回 Host 后在 Agent 运行页加载运行后强制下线无法直达 Agent 运行页/u);
  assert.match(source, /重复 clientRequestId 创建 Agent Run 幂等返回同一 runId/u);
  assert.match(source, /切租户后重复 clientRequestId 创建 Agent Run 幂等返回同一 runId/u);
  ...
  assert.match(source, /Host 上下文幂等创建并取消后无法重复取消 Agent Run/u);
  assert.match(source, /切租户后幂等创建并取消后无法重复取消 Agent Run/u);
  assert.match(source, /切租户并返回 Host 后幂等创建并取消后无法重复取消 Agent Run/u);
  ...
  assert.match(source, /切租户后强制下线后已失效 access token 无法触发后台任务/u);
  assert.match(source, /切租户并返回 Host 后退出后已失效 access token 无法触发后台任务/u);
  assert.match(source, /切租户并返回 Host 后退出后已失效 access token 无法访问后台任务执行历史 API/u);
  assert.match(source, /切租户后强制下线后已失效 access token 无法访问后台任务执行历史 API/u);
  assert.match(source, /切租户并返回 Host 后强制下线后已失效 access token 无法触发后台任务/u);
  assert.match(source, /切租户并返回 Host 后强制下线后已失效 access token 无法访问后台任务执行历史 API/u);
  assert.match(source, /切租户后退出后已失效 access token 无法访问工作流待办 API/u);
  assert.match(source, /切租户后退出后已失效 access token 无法访问 \/api\/v1\/ai\/agent-tools/u);
  assert.match(source, /切租户后退出后已失效 access token 无法访问后台任务定义 API/u);
  assert.match(source, /切租户后强制下线后已失效 access token 无法访问工作流待办 API/u);
  assert.match(source, /切租户后强制下线后已失效 access token 无法访问 \/api\/v1\/ai\/agent-tools/u);
  assert.match(source, /切租户并返回 Host 后退出后已失效 access token 无法访问工作流待办 API/u);
  assert.match(source, /切租户并返回 Host 后退出后已失效 access token 无法访问 \/api\/v1\/ai\/agent-tools/u);
  assert.match(source, /切租户后强制下线后已失效 access token 无法访问后台任务定义 API/u);
  assert.match(source, /切租户并返回 Host 后退出后已失效 access token 无法访问后台任务定义 API/u);
  assert.match(source, /切租户并返回 Host 后强制下线后已失效 access token 无法访问工作流待办 API/u);
  assert.match(source, /切租户并返回 Host 后强制下线后已失效 access token 无法访问 \/api\/v1\/ai\/agent-tools/u);
  assert.match(source, /切租户并返回 Host 后强制下线后已失效 access token 无法访问后台任务定义 API/u);
  assert.match(source, /切租户后退出后无法直接访问受保护路由/u);
  assert.match(source, /切租户后强制下线后无法直接访问受保护路由/u);
  assert.match(source, /切租户并返回 Host 后退出后无法直接访问受保护路由/u);
  assert.match(source, /切租户并返回 Host 后强制下线后无法直接访问受保护路由/u);
  assert.match(source, /切租户后应用退出后写回 refresh 凭据仍无法恢复会话/u);
  assert.match(source, /切租户后应用退出后清理本地凭据、中心 Cookie 并拒绝 refresh token/u);
  assert.match(source, /切租户后强制下线后清理本地凭据、中心 Cookie 并拒绝 refresh token/u);
  assert.match(source, /切租户后强制下线后写回 refresh 凭据仍无法恢复会话/u);
  assert.match(source, /切租户并返回 Host 后应用退出后清理本地凭据、中心 Cookie 并拒绝 refresh token/u);
  assert.match(source, /切租户并返回 Host 后强制下线后清理本地凭据、中心 Cookie 并拒绝 refresh token/u);
  assert.match(source, /切租户并返回 Host 后应用退出后写回 refresh 凭据仍无法恢复会话/u);
  assert.match(source, /切租户并返回 Host 后强制下线后写回 refresh 凭据仍无法恢复会话/u);
  assert.match(source, /切租户后上下文可完成工作流待办同意/u);
  assert.match(source, /切租户后上下文可完成工作流待办驳回/u);
  assert.match(source, /切租户并返回 Host 后上下文可完成工作流待办同意/u);
  ...
  assert.match(source, /可通过 Agent 运行页 UI 加载排队运行/u);
  assert.match(source, /切租户后可通过 Agent 运行页 UI 加载排队运行/u);
  assert.match(source, /切租户并返回 Host 后可通过 Agent 运行页 UI 加载排队运行/u);
  assert.match(source, /可通过 Agent 运行页 UI 取消排队运行/u);
  assert.match(source, /切租户后可通过 Agent 运行页 UI 取消排队运行/u);
  assert.match(source, /切租户并返回 Host 后可通过 Agent 运行页 UI 取消排队运行/u);
  assert.match(source, /ai-agent-runs-cancel/u);
  assert.match(source, /切租户后可通过 Agent 运行页 UI 创建排队运行/u);
  ...
  assert.match(source, /切租户并返回 Host 后 access token 仍可创建并读取排队 Agent Run/u);
  assert.match(source, /切租户并返回 Host 后可通过 Agent 运行页 UI 创建排队运行/u);
  assert.match(source, /Agent 工具/u);
  assert.match(source, /可打开 Agent 运行页/u);
  assert.match(source, /切租户后可打开 Agent 运行页/u);
  assert.match(source, /切租户并返回 Host 后可打开 Agent 运行页/u);
  assert.match(source, /ai-agent-runs-create/u);
