...
const contractPath = path.join(
  repositoryRoot,
  'contracts/openapi/identity-host-user-roles-v1.json'
);
const contractsSourcePath = path.join(
  repositoryRoot,
  'src/Modules/Full.NET.Modules.Identity.Contracts/HostUserRolesContracts.cs'
);
...
test('Host 用户角色 OpenAPI 夹具结构完整且路径唯一', async () => {
  const contract = await loadContract();
  assert.equal(contract.id, 'identity-host-user-roles-v1');
...
test('Host 用户角色 OpenAPI 夹具与 C# 契约和端点源码一致', async () => {
  const contract = await loadContract();
  const contractsSource = await readFile(contractsSourcePath, 'utf8');
  const endpointSource = await readFile(endpointSourcePath, 'utf8');

  assert.match(contractsSource, /record HostUserRolesResponse/u);
  assert.match(contractsSource, /record ReplaceHostUserRolesRequest/u);
  assert.match(endpointSource, /MapGet\("\/\{userId:guid\}\/roles"/u);
  assert.match(endpointSource, /MapPut\("\/\{userId:guid\}\/roles"/u);
