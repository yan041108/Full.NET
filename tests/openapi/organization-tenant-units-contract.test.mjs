...
test('租户机构 OpenAPI 夹具结构完整且路径唯一', async () => {
  ...
  for (const entry of contract.paths) {
    ...
    for (const operation of entry.operations) {
      ...
      seen.add(key);
      assert.match(operation.permission, /^organization\.units\.(read|write)$/u);
...
test('租户机构 OpenAPI 夹具与 C# 契约和端点源码一致', async () => {
  ...
  assert.match(contractsSource, /organization\.units\.read/u);
  assert.match(contractsSource, /organization\.units\.write/u);
