...
test('manifest 与规范快照精确登记生成操作且 CI 只执行离线 check', async () => {
  ...
  assert.equal(manifest.entries.length, 556);
  assert.equal(new Set(manifest.entries.map(entry => entry.operationId)).size, 556);
  assert.deepEqual(
    manifest.entries
  ...
  assert.deepEqual(
    ...
    [
      ...
      'document-host-tags',
      'document-host-version-retention',
      'enterprise-requests',
      'files-host-files',
      'files-host-folders',
      'goview-projects',
  ...
  assert.equal(
    manifest.entries.every(entry => ['generated', 'manual'].includes(entry.status)),
    true
  );
  assert.equal(
