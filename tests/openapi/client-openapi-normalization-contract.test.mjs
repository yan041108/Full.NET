...
test('manifest 与规范快照精确登记三个试点且 CI 只执行离线 check', async () => {
  ...
  assert.equal(manifest.entries.length, 58);
  assert.equal(new Set(manifest.entries.map(entry => entry.operationId)).size, 58);
  assert.deepEqual(
  ...
  assert.equal(
    manifest.entries
      .filter(entry => entry.generatedGroup === 'identity-host-modules')
