...
public sealed class AuthorizationCatalogTests
{
    ...
    public void Host_roles_actions_bind_to_exact_permissions()
    {
        ...
        var expected = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["enable"] = "identity.roles.enable",
            ["delete"] = "identity.roles.delete",
            ["replace-members"] = "identity.roles.replace_members",
    ...
    [TestMethod]
    public void Tenant_org_position_levels_actions_bind_to_exact_permissions()
    {
        var catalog = AuthorizationCatalog.Create(
            [new OrganizationAuthorizationContributor()]);
        ...
        var expected = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ["create"] = "organization.position_levels.create",
            ["update"] = "organization.position_levels.update",
            ["disable"] = "organization.position_levels.disable",
        };

        var levelActions = catalog.Actions
            .Where(action => action.NavigationId == "org-position-levels")
            .ToDictionary(
                action => action.ClientActionKey,
                action => action.PermissionCode,
                StringComparer.Ordinal);
        ...
        CollectionAssert.AreEquivalent(
            ...
            levelActions);
        Assert.IsFalse(catalog.Permissions.Any(
            permission => permission.Code == OrganizationPositionLevelManagementPermissions.Write));
    }

    [TestMethod]
    public void Tenant_org_user_positions_actions_bind_to_exact_permissions()
    {
        var catalog = AuthorizationCatalog.Create(
            [new OrganizationAuthorizationContributor()]);
        ...
        var expected = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ["create"] = "organization.user_positions.create",
            ["update"] = "organization.user_positions.update",
            ["disable"] = "organization.user_positions.disable",
        };

        var userPositionActions = catalog.Actions
            .Where(action => action.NavigationId == "org-user-positions")
            .ToDictionary(
                action => action.ClientActionKey,
                action => action.PermissionCode,
                StringComparer.Ordinal);
        ...
        CollectionAssert.AreEquivalent(
            ...
            userPositionActions);
        Assert.IsFalse(catalog.Permissions.Any(
            permission => permission.Code == OrganizationUserPositionManagementPermissions.Write));
    }

    [TestMethod]
    public void Tenant_org_user_units_actions_bind_to_exact_permissions()
    {
        var catalog = AuthorizationCatalog.Create(
            [new OrganizationAuthorizationContributor()]);
        ...
        var expected = new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ["create"] = "organization.user_units.create",
            ["update"] = "organization.user_units.update",
            ["disable"] = "organization.user_units.disable",
        };

        var userUnitActions = catalog.Actions
            .Where(action => action.NavigationId == "org-user-units")
            .ToDictionary(
                action => action.ClientActionKey,
                action => action.PermissionCode,
                StringComparer.Ordinal);
