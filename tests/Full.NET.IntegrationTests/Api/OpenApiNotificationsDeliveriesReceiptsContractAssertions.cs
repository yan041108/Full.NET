...
/// <summary>校验投递查询、人工重试与匿名回执端点在 OpenAPI 文档中的路径与核心 schema。</summary>
internal static class OpenApiNotificationsDeliveriesReceiptsContractAssertions
{
    ...
    private static void AssertPilotOperations(JsonElement document)
    {
        ...
        OpenApiPilotContractAssertions.AssertOperation(
            document,
            "/api/v1/notifications/deliveries/{deliveryId}",
            HttpMethod.Get,
        ...
        while (directory is not null)
        {
            if (File.Exists(Path.Combine(directory.FullName, "Full.NET.slnx")))
            {
                await using var stream = File.OpenRead(Path.Combine(
                    ...
                    "contracts",
                    "openapi",
                    "notifications-deliveries-receipts-v1.json"));
