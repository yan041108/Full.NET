...
public sealed class SettingsApiSqlServerTests
{
    ...
    [TestMethod]
    public async Task Tenant_dict_type_management_follows_contract_with_sql_server()
    {
        using var factory = new FullNetApiFactory(
            DatabaseProvider.SqlServer,
            await SharedDatabaseFixture.CreateSqlServerDatabaseAsync());
    ...
    [TestMethod]
    public async Task Current_user_grid_preferences_follow_contract_with_sql_server()
    {
        using var factory = new FullNetApiFactory(
            DatabaseProvider.SqlServer,
            await SharedDatabaseFixture.CreateSqlServerDatabaseAsync());
    ...
    [TestMethod]
    public async Task Host_diagnostic_policy_management_follows_contract_with_sql_server()
    {
        using var factory = new FullNetApiFactory(
            DatabaseProvider.SqlServer,
            await SharedDatabaseFixture.CreateSqlServerDatabaseAsync());

        await DiagnosticPolicyAssertions.VerifyAsync(factory);
    }
}