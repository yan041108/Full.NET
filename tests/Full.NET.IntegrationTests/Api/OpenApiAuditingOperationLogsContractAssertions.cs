...
/// <summary>
/// 校验 Host 操作日志端点在 OpenAPI 文档中的路径与核心 schema。
/// </summary>
internal static class OpenApiAuditingOperationLogsContractAssertions
{
    ...
    {
        const string tag = "AuditingHostOperationLogs";
        OpenApiPilotContractAssertions.AssertOperation(
            document,
            "/api/v1/auditing/operation-logs",
            HttpMethod.Get,
        ...
        out JsonElement schema)
    {
        ...
        {
            if (candidate.Name.Contains("OperationLog", StringComparison.Ordinal))
            {
        ...
        while (directory is not null)
        {
            ...
            if (File.Exists(sln))
            {
                var contractPath = Path.Combine(
                    ...
                    "contracts",
                    "openapi",
                    "auditing-operation-logs-v1.json");
