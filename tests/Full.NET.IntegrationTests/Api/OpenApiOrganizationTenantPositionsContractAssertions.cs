...
/// <summary>
/// 校验租户职位管理端点在 OpenAPI 文档中的路径、方法与核心 schema 属性。
/// </summary>
internal static class OpenApiOrganizationTenantPositionsContractAssertions
{
        ...
        JsonElement contractSchema)
    {
        ...
        foreach (var property in contractSchema.GetProperty("properties").EnumerateArray())
        {
            var name = property.GetString()
                ?? throw new InvalidOperationException("Schema property name is required.");
            Assert.IsTrue(
                openApiProperties.TryGetProperty(name, out _),
                $"OpenAPI schema {schemaName} 缺少属性：{name}");
        }
    }
        ...
        out JsonElement schema)
    {
        ...
        if (schemaName == "OrganizationPositionResponsePage")
        {
            ...
            {
                if (!candidate.Name.Contains("OrganizationPositionResponse", StringComparison.Ordinal))
                {
                    continue;
                }
        ...
        foreach (var candidate in openApiSchemas.EnumerateObject())
        {
            if (candidate.Name.EndsWith(schemaName, StringComparison.Ordinal)
                || candidate.Name.Contains("OrganizationPosition", StringComparison.Ordinal))
            {
                schema = candidate.Value;
                return true;
            }
        ...
        while (directory is not null)
        {
            if (File.Exists(Path.Combine(directory.FullName, "Full.NET.slnx")))
            {
                var contractPath = Path.Combine(
                    ...
                    "contracts",
                    "openapi",
                    "organization-tenant-positions-v1.json");
