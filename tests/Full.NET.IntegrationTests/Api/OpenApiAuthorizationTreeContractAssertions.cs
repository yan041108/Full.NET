...
/// <summary>
/// 校验 Host 角色授权树端点在 OpenAPI 文档中的路径、方法与核心 schema 属性。
/// </summary>
internal static class OpenApiAuthorizationTreeContractAssertions
{
        ...
        out JsonElement schema)
    {
        ...
        if (schemaName == "AuthorizationTreeModuleResponseCollection")
        {
            foreach (var candidate in openApiSchemas.EnumerateObject())
            {
                ...
                if (candidate.Value.TryGetProperty("type", out var type)
        ...
        CancellationToken cancellationToken)
    {
        ...
        var contractPath = Path.Combine(
            ...
            "contracts",
            "openapi",
            "identity-authorization-tree-v1.json");
