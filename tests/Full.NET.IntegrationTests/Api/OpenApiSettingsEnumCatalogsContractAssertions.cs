...
/// <summary>
/// 校验 Host 枚举/常量目录端点在 OpenAPI 文档中的路径与核心 schema。
/// </summary>
internal static class OpenApiSettingsEnumCatalogsContractAssertions
{
        ...
        out JsonElement schema)
    {
        ...
        foreach (var candidate in openApiSchemas.EnumerateObject())
        {
            if (candidate.Name.EndsWith(schemaName, StringComparison.Ordinal)
                || candidate.Name.Contains("EnumCatalog", StringComparison.Ordinal))
            {
                if (schemaName.EndsWith("Summary", StringComparison.Ordinal)
                    && candidate.Name.Contains("Summary", StringComparison.Ordinal))
                {
                    schema = candidate.Value;
                    return true;
                }

                if (schemaName.EndsWith("Detail", StringComparison.Ordinal)
                    && candidate.Name.Contains("Detail", StringComparison.Ordinal))
                {
                    schema = candidate.Value;
                    return true;
                }

                if (schemaName == "EnumCatalogMember"
                    && candidate.Name.Contains("Member", StringComparison.Ordinal)
                    && !candidate.Name.Contains("Definition", StringComparison.Ordinal))
                {
                    schema = candidate.Value;
                    return true;
                }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var contractPath = Path.Combine(
            ...
            "contracts",
            "openapi",
            "settings-enum-catalogs-v1.json");
