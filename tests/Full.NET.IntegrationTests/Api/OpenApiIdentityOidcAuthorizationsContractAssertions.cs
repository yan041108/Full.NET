...
internal static class OpenApiIdentityOidcAuthorizationsContractAssertions
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var contractPath in contractPaths.EnumerateArray())
        {
            ...
            Assert.IsTrue(openApiPaths.TryGetProperty(path, out var openApiPath), $"OpenAPI missing path: {path}");
            foreach (var operation in contractPath.GetProperty("operations").EnumerateArray())
            {
                ...
                Assert.IsTrue(
                    openApiPath.TryGetProperty(method, out _),
                    $"OpenAPI missing operation: {method.ToUpperInvariant()} {path}");
            }
        }
    }

    private static async Task<JsonDocument> LoadContractDocumentAsync(CancellationToken cancellationToken)
    {
        ...
        var contractPath = Path.Combine(
            ...
            "contracts",
            "openapi",
            "identity-oidc-authorizations-v1.json");
