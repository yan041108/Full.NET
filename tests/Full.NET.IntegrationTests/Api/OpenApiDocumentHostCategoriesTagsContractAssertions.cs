...
/// <summary>
/// 校验 Host 文档分类与标签端点在 OpenAPI 文档中的路径、方法与核心 schema 属性。
/// </summary>
internal static class OpenApiDocumentHostCategoriesTagsContractAssertions
{
        ...
        foreach (var contractPath in contractPaths.EnumerateArray())
        {
            ...
            foreach (var operation in contractPath.GetProperty("operations").EnumerateArray())
            {
                ...
                if (operation.TryGetProperty("responseSchema", out var responseSchema))
                {
                    ...
                    if (responseSchemaName.EndsWith("List", StringComparison.Ordinal))
                    {
                        continue;
                    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var contractPath = Path.Combine(
            ...
            "contracts",
            "openapi",
            "document-host-categories-tags-v1.json");
