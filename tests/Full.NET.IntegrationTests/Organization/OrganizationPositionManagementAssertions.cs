...
internal static class OrganizationPositionManagementAssertions
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await VerifyExactOrganizationPositionActionPermissionBoundariesAsync(
            client,
            cancellationToken);
    ...
    private static async Task VerifyExactOrganizationPositionActionPermissionBoundariesAsync(
        HttpClient client,
        CancellationToken cancellationToken)
    {
        var adminTenantToken = await LoginAndEnterAcmeTenantAsync(client, cancellationToken);
        ...
        using var createRequest = CreateBearerJsonRequest(
            ...
            "/api/v1/organization/positions",
            adminTenantToken,
            new CreateOrganizationPositionRequest(code, "边界测试职位", 10));
        using var createResponse = await client.SendAsync(createRequest, cancellationToken);
        Assert.AreEqual(HttpStatusCode.Created, createResponse.StatusCode);
        var created = await createResponse.Content.ReadFromJsonAsync<OrganizationPositionResponse>(
            cancellationToken);
        Assert.IsNotNull(created);
        ...
        using var disableTargetRequest = CreateBearerJsonRequest(
            ...
            "/api/v1/organization/positions",
            adminTenantToken,
            new CreateOrganizationPositionRequest(disableCode, "禁用边界职位", 10));
        ...
        var readOnlyToken = await EnterAcmeTenantWithRolePermissionsAsync(
            ...
            cancellationToken);
        await AssertOrganizationPositionPermissionDeniedAsync(
            client,
            readOnlyToken,
            HttpMethod.Post,
            "/api/v1/organization/positions",
            cancellationToken,
            new CreateOrganizationPositionRequest("denied", "拒绝", 10));
        await AssertOrganizationPositionPermissionDeniedAsync(
            client,
            readOnlyToken,
            HttpMethod.Put,
        ...
        using var disableRequest = CreateBearerJsonRequest(
            HttpMethod.Post,
            $"/api/v1/organization/positions/{disableTarget.Id:D}/disable",
            disableToken,
            new { });
    ...
    private static async Task AssertOrganizationPositionPermissionDeniedAsync(
        HttpClient client,
        string accessToken,
        HttpMethod method,
        string path,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var roleCode = $"pos-bound-{Guid.NewGuid():N}".ToLowerInvariant();
        var username = $"pos-bound-{Guid.NewGuid():N}".ToLowerInvariant();
        ...
        using var createRoleRequest = CreateBearerJsonRequest(
            ...
            hostAdminToken,
            new CreateHostRoleRequest(roleCode, "职位动作边界角色"));
