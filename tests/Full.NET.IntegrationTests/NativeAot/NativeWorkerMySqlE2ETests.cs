...
[TestClass]
[DoNotParallelize]
public sealed class NativeWorkerMySqlE2ETests
{
    [TestMethod]
    public async Task MySql_native_worker_runs_version_retirement_scan()
    {
    ...
    [TestMethod]
    public async Task MySql_native_worker_runs_persistent_background_runtime()
    {
        if (!NativeWorkerArtifactLocator.TryResolve(out _, out var skipReason))
        {
            Assert.Inconclusive(skipReason ?? "Native Worker artifact unavailable.");
    ...
    [TestMethod]
    public async Task MySql_native_worker_processes_legacy_outbox_terminal_states()
    {
        if (!NativeWorkerArtifactLocator.TryResolve(out _, out var skipReason))
        {
            Assert.Inconclusive(skipReason ?? "Native Worker artifact unavailable.");
    ...
    [TestMethod]
    public async Task MySql_native_worker_projects_workflow_notification_to_inbox()
    {
        if (!NativeWorkerArtifactLocator.TryResolve(out _, out var skipReason))
        {
            Assert.Inconclusive(skipReason ?? "Native Worker artifact unavailable.");
    ...
    [TestMethod]
    public async Task MySql_native_worker_processes_pending_ping_job()
    {
        if (!NativeWorkerArtifactLocator.TryResolve(out _, out var skipReason))
        {
            Assert.Inconclusive(skipReason ?? "Native Worker artifact unavailable.");
    ...
    [TestMethod]
    public async Task MySql_native_worker_cleans_deleted_local_files()
    {
        if (!NativeWorkerArtifactLocator.TryResolve(out _, out var skipReason))
        {
            Assert.Inconclusive(skipReason ?? "Native Worker artifact unavailable.");
    ...
    [TestMethod]
    public async Task MySql_native_worker_reconciles_pending_file_reference_claims()
    {
        if (!NativeWorkerArtifactLocator.TryResolve(out _, out var skipReason))
        {
            Assert.Inconclusive(skipReason ?? "Native Worker artifact unavailable.");
