...
/// <summary>
/// 验证 071 将存量 files.files.write 展开为 upload/delete、为存量 files.files.read 补齐 download，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration071FilesActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "files.files.write";
    private const string LegacyRead = "files.files.read";
    private const string ActionUpload = "files.files.upload";
    private const string ActionDelete = "files.files.delete";
    private const string ActionDownload = "files.files.download";
    ...
    [TestMethod]
    public async Task SqlServer_adds_download_to_legacy_read_grants()
    {
        var connectionString = await SharedDatabaseFixture.CreateSqlServerDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.SqlServer, connectionString);
        await runner.MigrateAsync();
    ...
    public async Task SqlServer_deduplicates_existing_action_grants()
    {
        ...
        await SeedSqlServerStateAsync(
            ...
            legacyPermission: LegacyWrite,
            apiKeyPermissionsJson:
                $$"""["{{LegacyWrite}}","{{ActionUpload}}","{{UnrelatedPermission}}","{{LegacyWrite}}","{{ActionUpload}}"]""");
    ...
    public async Task SqlServer_partial_expansion_converges_on_rerun()
    {
        ...
        await InsertRoleActionGrantsAsync(connection, roleId, isSqlServer: true, ActionUpload);
    ...
    [TestMethod]
    public async Task MySql_adds_download_to_legacy_read_grants()
    {
        var connectionString = await SharedDatabaseFixture.CreateMySqlDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.MySql, connectionString);
        await runner.MigrateAsync();
    ...
    public async Task MySql_partial_expansion_converges_on_rerun()
    {
        ...
        await SeedMySqlStateAsync(
            ...
            legacyPermission: LegacyWrite,
            apiKeyPermissionsJson: $$"""["{{LegacyWrite}}"]""");
        await InsertRoleActionGrantsAsync(connection, roleId, isSqlServer: false, ActionUpload);
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%071_FilesActionPermissions.sql';
                  """
                : """
