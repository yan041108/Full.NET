...
/// <summary>
/// 验证 057 将存量 identity.menus.write 展开为三个精确 Host 菜单动作权限，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration057IdentityMenuActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "identity.menus.write";
    private const string ActionCreate = "identity.menus.create";
        ...
        string apiKeyPermissionsJson)
    {
        await connection.ExecuteAsync(
            ...
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, N'Menu Legacy', 0, 1,
                 0, @Now, NULL, 1);
            ...
            VALUES
                (@ApiKeyId, @UserId, N'Legacy Menu Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
                RoleId = roleId,
                RoleCode = $"menu-legacy-{roleId:N}"[..24],
                UserId = userId,
                Username = $"mlegacy-{userId:N}"[..24],
                NormalizedUsername = $"MLEGACY-{userId:N}"[..24],
        ...
        await connection.ExecuteAsync(
            ...
            INSERT INTO fn_identity_role
                (Id, TenantId, ScopeKey, Code, Name, IsSystem, IsActive,
                 IsSuperAdministrator, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, 'Menu Legacy', 0, 1,
                 0, @Now, NULL, 1);
            ...
            INSERT INTO fn_identity_api_key
                (Id, UserId, DisplayName, KeyPrefix, KeyHash, PermissionsJson,
                 ...
                 CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@ApiKeyId, @UserId, 'Legacy Menu Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%057_IdentityMenuActionPermissions.sql';
                  """
                : """
