...
/// <summary>
/// 验证 049 在 <c>fn_settings_domain_audit</c> 已存在但 TenantId 复合索引缺失或形状错误时能够无损恢复。
/// </summary>
[TestClass]
public sealed class Migration050SettingsDomainAuditRecoveryTests
{
    private const string IndexName =
        "IX_fn_settings_domain_audit_TenantId_OccurredAtUtc_Id";
    ...
    public async Task SqlServer_domain_audit_migration_recovers_missing_index_without_dropping_data()
    {
        ...
        await connection.ExecuteAsync(
            $"""
             INSERT INTO dbo.fn_settings_domain_audit
                 (Id, TenantId, ActionKey, EntityId, Outcome, ActorUserId,
                  ActorDisplayName, TraceId, DiffSummaryJson, OccurredAtUtc)
             VALUES
                 (NEWID(), NULL, 'settings.logging-diagnostic-policy.updated', NEWID(), 'success',
                  NULL, NULL, NULL, NULL, SYSDATETIMEOFFSET());
             DROP INDEX {IndexName} ON dbo.fn_settings_domain_audit;
             DELETE FROM dbo.SchemaVersions
             WHERE ScriptName LIKE '%050_SettingsDomainAudit.sql';
             """);
        ...
        Assert.AreEqual(1, await connection.ExecuteScalarAsync<int>(
            """
            SELECT COUNT(*)
            FROM dbo.fn_settings_domain_audit
    ...
    public async Task MySql_domain_audit_migration_recovers_missing_index_without_dropping_data()
    {
            ...
            $"""
             INSERT INTO fn_settings_domain_audit
                 (Id, TenantId, ActionKey, EntityId, Outcome, ActorUserId,
                  ActorDisplayName, TraceId, DiffSummaryJson, OccurredAtUtc)
             VALUES
                 (UNHEX(REPLACE(UUID(), '-', '')), NULL, 'settings.logging-diagnostic-policy.updated',
                  UNHEX(REPLACE(UUID(), '-', '')), 'success', NULL, NULL, NULL, NULL,
                  UTC_TIMESTAMP(6));
             DROP INDEX {IndexName} ON fn_settings_domain_audit;
        ...
        await connection.ExecuteAsync(
            $"""
             DROP INDEX {IndexName} ON fn_settings_domain_audit;
             CREATE INDEX {IndexName}
                 ON fn_settings_domain_audit (OccurredAtUtc, Id);
             DELETE FROM schemaversions
             WHERE ScriptName LIKE '%050_SettingsDomainAudit.sql';
             """);
        ...
        Assert.AreEqual(3, await connection.ExecuteScalarAsync<int>(
            $"""
             ...
             WHERE indexObject.object_id =
                   OBJECT_ID(N'dbo.fn_settings_domain_audit')
