...
/// <summary>
/// 验证 055 将存量 identity.roles.write 展开为五个精确 Host 角色动作权限，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration055IdentityRoleActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "identity.roles.write";
    private const string ActionCreate = "identity.roles.create";
    private const string ActionUpdate = "identity.roles.update";
    private const string ActionAssignPermissions = "identity.roles.assign_permissions";
    private const string ActionDisable = "identity.roles.disable";
    private const string ActionAssignDataScope = "identity.roles.assign_data_scope";
    ...
    [
        ActionCreate,
        ActionUpdate,
        ActionAssignPermissions,
        ActionDisable,
        ActionAssignDataScope,
    ];
    ...
    public async Task SqlServer_deduplicates_existing_action_grants()
    {
        ...
        await SeedSqlServerStateAsync(
            ...
            apiKeyId,
            now,
            insertExistingActionGrants: true,
            apiKeyPermissionsJson:
    ...
    public async Task SqlServer_partial_expansion_converges_on_rerun()
    {
        ...
        await InsertRoleActionGrantsAsync(connection, roleId, isSqlServer: true, ActionCreate, ActionUpdate);
    ...
    public async Task MySql_partial_expansion_converges_on_rerun()
    {
        ...
        await SeedMySqlStateAsync(
            ...
            insertExistingActionGrants: false,
            apiKeyPermissionsJson: $$"""["{{LegacyWrite}}"]""");
        await InsertRoleActionGrantsAsync(connection, roleId, isSqlServer: false, ActionCreate, ActionUpdate);
        ...
        string apiKeyPermissionsJson)
    {
        await connection.ExecuteAsync(
            ...
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, N'Role Action Legacy', 0, 1,
                 0, @Now, NULL, 1);
            ...
            VALUES
                (@UserId, NULL, 'host', @Username, @NormalizedUsername, N'API Role User',
            ...
            VALUES
                (@ApiKeyId, @UserId, N'Legacy Role Action Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
                RoleId = roleId,
                RoleCode = $"role-action-{roleId:N}"[..24],
                UserId = userId,
                Username = $"ract-{userId:N}"[..24],
                NormalizedUsername = $"RACT-{userId:N}"[..24],
        ...
        await connection.ExecuteAsync(
            ...
            INSERT INTO fn_identity_role
                (Id, TenantId, ScopeKey, Code, Name, IsSystem, IsActive,
                 IsSuperAdministrator, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, 'Role Action Legacy', 0, 1,
                 0, @Now, NULL, 1);
            INSERT INTO fn_identity_user
                (Id, TenantId, ScopeKey, Username, NormalizedUsername, DisplayName,
                 ...
                 SecurityStamp, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@UserId, NULL, 'host', @Username, @NormalizedUsername, 'API Role User',
                 'unused', 1, 0, NULL, @SecurityStamp, @Now, NULL, 1);
            ...
            INSERT INTO fn_identity_api_key
                (Id, UserId, DisplayName, KeyPrefix, KeyHash, PermissionsJson,
                 ...
                 CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@ApiKeyId, @UserId, 'Legacy Role Action Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%055_IdentityRoleActionPermissions.sql';
                  """
                : """
