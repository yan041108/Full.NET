...
/// <summary>验证 137 为全部存量角色幂等补齐行政区域动作权限。</summary>
[TestClass]
public sealed class Migration137RegionsAdministrativeRegionActionPermissionsRecoveryTests
{
    ...
    [TestMethod]
    public async Task SqlServer_grants_all_regions_permissions_to_existing_roles()
    {
        ...
        await SeedRoleWithoutRegionsPermissionsAsync(connection, roleId, now, isSqlServer: true);
    ...
    [TestMethod]
    public async Task SqlServer_deduplicates_existing_regions_permissions()
    {
        var connectionString = await SharedDatabaseFixture.CreateSqlServerDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.SqlServer, connectionString);
        await runner.MigrateAsync();
        ...
        await connection.ExecuteAsync(
            ...
            """,
            new { RoleId = roleId, PermissionCode = RegionsPermissions.Read });
    ...
    [TestMethod]
    public async Task MySql_grants_all_regions_permissions_to_existing_roles()
    {
        ...
        var roleId = Guid.NewGuid();
        var now = DateTimeOffset.UtcNow;
        await SeedRoleWithoutRegionsPermissionsAsync(connection, roleId, now, isSqlServer: false);
    ...
    [TestMethod]
    public async Task MySql_deduplicates_existing_regions_permissions()
    {
        var connectionString = await SharedDatabaseFixture.CreateMySqlDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.MySql, connectionString);
        await runner.MigrateAsync();
    ...
    private static Task SeedRoleWithoutRegionsPermissionsAsync(
        ...
        bool isSqlServer)
    {
        var sql = isSqlServer
            ? """
              ...
              VALUES
                  (@RoleId, NULL, 'host', @RoleCode, N'Regions Seed Role', 0, 1,
                   0, @Now, NULL, 1);
              """
            : """
              INSERT INTO fn_identity_role
                  (Id, TenantId, ScopeKey, Code, Name, IsSystem, IsActive,
                   IsSuperAdministrator, CreatedAtUtc, UpdatedAtUtc, Version)
              VALUES
                  (@RoleId, NULL, 'host', @RoleCode, 'Regions Seed Role', 0, 1,
                   0, @Now, NULL, 1);
              """;
        return connection.ExecuteAsync(
            ...
            {
                RoleId = roleId,
                RoleCode = $"regions-seed-{roleId:N}"[..24],
                Now = now,
            });
    }
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%137_RegionsAdministrativeRegionActionPermissions.sql';
                  """
                : """
        ...
        params string[] expectedPermissions)
    {
        ...
        var permissions = (await connection.QueryAsync<string>(
            $"""
             ...
             FROM {tableName}
             WHERE RoleId = @RoleId
               AND PermissionCode LIKE 'regions.administrative_regions.%'
