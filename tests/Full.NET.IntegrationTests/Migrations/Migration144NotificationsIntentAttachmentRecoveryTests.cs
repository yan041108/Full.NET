...
/// <summary>验证 144 通知 Intent 附件投影表可幂等恢复。</summary>
[TestClass]
public sealed class Migration144NotificationsIntentAttachmentRecoveryTests
{
    [TestMethod]
    public async Task SqlServer_recreates_intent_attachment_table()
    {
        ...
        await DropTableAsync(connection, isSqlServer: true);
    ...
    [TestMethod]
    public async Task MySql_recreates_intent_attachment_table()
    {
        ...
        await DropTableAsync(connection, isSqlServer: false);
    ...
    {
        var sql = isSqlServer
            ? "DROP TABLE IF EXISTS dbo.fn_notifications_intent_attachment;"
            : "DROP TABLE IF EXISTS fn_notifications_intent_attachment;";
    ...
    {
        var sql = isSqlServer
            ? "SELECT COUNT(1) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'fn_notifications_intent_attachment'"
            : """
              SELECT COUNT(1)
              FROM information_schema.tables
              WHERE table_schema = DATABASE()
                AND table_name = 'fn_notifications_intent_attachment'
              """;
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%144_NotificationsIntentAttachment.sql';
                  """
                : """
