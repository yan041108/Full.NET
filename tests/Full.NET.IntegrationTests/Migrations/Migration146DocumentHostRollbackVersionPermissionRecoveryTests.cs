...
/// <summary>验证 146 为已具备 update 权限的角色与 API Key 幂等授予 rollback_version 权限。</summary>
[TestClass]
public sealed class Migration146DocumentHostRollbackVersionPermissionRecoveryTests
{
    private const string UpdatePermission = "document.host_documents.update";
    private const string RollbackPermission = "document.host_documents.rollback_version";
    private const string UnrelatedPermission = "identity.roles.read";

    [TestMethod]
    public async Task SqlServer_grants_rollback_version_to_update_role_and_api_key()
    {
        ...
        await SeedSqlServerStateAsync(
            ...
            apiKeyId,
            now,
            apiKeyPermissionsJson: $$"""["{{UpdatePermission}}","{{UnrelatedPermission}}"]""");
    ...
    [TestMethod]
    public async Task MySql_grants_rollback_version_to_update_role_and_api_key()
    {
        ...
        string apiKeyPermissionsJson)
    {
        await connection.ExecuteAsync(
            ...
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, N'Unit Update', 0, 1,
                 0, @Now, NULL, 1);
            ...
            VALUES
                (@ApiKeyId, @UserId, N'Update Package Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
                RoleId = roleId,
                RoleCode = $"pkg-update-{roleId:N}"[..24],
                UserId = userId,
                Username = $"uupdate-{userId:N}"[..24],
                NormalizedUsername = $"uupdate-{userId:N}"[..24],
        ...
        await connection.ExecuteAsync(
            ...
            INSERT INTO fn_identity_role
                (Id, TenantId, ScopeKey, Code, Name, IsSystem, IsActive,
                 IsSuperAdministrator, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, 'Unit Update', 0, 1,
                 0, @Now, NULL, 1);
            INSERT INTO fn_identity_user
                (Id, TenantId, ScopeKey, Username, NormalizedUsername, DisplayName,
                 ...
                 SecurityStamp, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@UserId, NULL, 'host', @Username, @NormalizedUsername, 'Update User',
                 'unused', 1, 0, NULL, @SecurityStamp, @Now, NULL, 1);
            ...
            INSERT INTO fn_identity_api_key
                (Id, UserId, DisplayName, KeyPrefix, KeyHash, PermissionsJson,
                 ...
                 CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@ApiKeyId, @UserId, 'Update Package Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%146_DocumentHostRollbackVersionPermission.sql';
                  """
                : """
