...
/// <summary>
/// 验证 058 将存量 identity.sessions.write 展开为 identity.sessions.revoke，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration058IdentitySessionActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "identity.sessions.write";
    private const string ActionRevoke = "identity.sessions.revoke";
    ...
    public async Task SqlServer_deduplicates_existing_action_grants()
    {
        ...
        await SeedSqlServerStateAsync(
            ...
            insertExistingActionGrants: false,
            apiKeyPermissionsJson:
                $$"""["{{LegacyWrite}}","{{ActionRevoke}}","{{UnrelatedPermission}}","{{LegacyWrite}}","{{ActionRevoke}}"]""");
    ...
    public async Task SqlServer_partial_expansion_converges_on_rerun()
    {
        ...
        await InsertRoleActionGrantsAsync(connection, roleId, isSqlServer: true, ActionRevoke);
    ...
    public async Task MySql_partial_expansion_converges_on_rerun()
    {
        ...
        await SeedMySqlStateAsync(
            ...
            insertExistingActionGrants: false,
            apiKeyPermissionsJson: $$"""["{{LegacyWrite}}"]""");
        await InsertRoleActionGrantsAsync(connection, roleId, isSqlServer: false, ActionRevoke);
        ...
        string apiKeyPermissionsJson)
    {
        await connection.ExecuteAsync(
            ...
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, N'Session Legacy', 0, 1,
                 0, @Now, NULL, 1);
            ...
            VALUES
                (@UserId, NULL, 'host', @Username, @NormalizedUsername, N'API Session User',
            ...
            VALUES
                (@ApiKeyId, @UserId, N'Legacy Session Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
                RoleId = roleId,
                RoleCode = $"session-legacy-{roleId:N}"[..24],
                UserId = userId,
                Username = $"slegacy-{userId:N}"[..24],
                NormalizedUsername = $"SLEGACY-{userId:N}"[..24],
        ...
        if (insertExistingActionGrants)
        {
            await connection.ExecuteAsync(
                """
                INSERT INTO dbo.fn_identity_role_permission (RoleId, PermissionCode)
                VALUES (@RoleId, @ActionRevoke);
                """,
                new { RoleId = roleId, ActionRevoke });
        }
    }
        ...
        await connection.ExecuteAsync(
            ...
            INSERT INTO fn_identity_role
                (Id, TenantId, ScopeKey, Code, Name, IsSystem, IsActive,
                 IsSuperAdministrator, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, 'Session Legacy', 0, 1,
                 0, @Now, NULL, 1);
            INSERT INTO fn_identity_user
                (Id, TenantId, ScopeKey, Username, NormalizedUsername, DisplayName,
                 ...
                 SecurityStamp, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@UserId, NULL, 'host', @Username, @NormalizedUsername, 'API Session User',
                 'unused', 1, 0, NULL, @SecurityStamp, @Now, NULL, 1);
            ...
            INSERT INTO fn_identity_api_key
                (Id, UserId, DisplayName, KeyPrefix, KeyHash, PermissionsJson,
                 ...
                 CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@ApiKeyId, @UserId, 'Legacy Session Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%058_IdentitySessionActionPermissions.sql';
                  """
                : """
