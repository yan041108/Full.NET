...
/// <summary>
/// 验证 066 将存量 organization.user_units.write 展开为三个精确租户用户机构隶属动作权限，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration066OrganizationUserUnitActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "organization.user_units.write";
    private const string ActionCreate = "organization.user_units.create";
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%066_OrganizationUserUnitActionPermissions.sql';
                  """
                : """
