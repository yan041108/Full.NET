...
/// <summary>
/// 验证 077 将存量 serial_numbers.rules.write 展开为四个精确 Host 流水号规则动作权限，并为存量 read 补齐 preview，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration077SerialNumberRuleActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "serial_numbers.rules.write";
    private const string ActionCreate = "serial_numbers.rules.create";
    private const string UnrelatedPermission = "identity.roles.read";

    private const string LegacyRead = "serial_numbers.rules.read";
    private const string ActionPreview = "serial_numbers.rules.preview";
    ...
    [TestMethod]
    public async Task SqlServer_adds_preview_to_legacy_read_grants()
    {
        var connectionString = await SharedDatabaseFixture.CreateSqlServerDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.SqlServer, connectionString);
        await runner.MigrateAsync();
    ...
    [TestMethod]
    public async Task MySql_adds_preview_to_legacy_read_grants()
    {
        var connectionString = await SharedDatabaseFixture.CreateMySqlDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.MySql, connectionString);
        await runner.MigrateAsync();
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%077_SerialNumberRuleActionPermissions.sql';
                  """
                : """
