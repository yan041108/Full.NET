...
/// <summary>
/// 验证 069 将存量 settings.config.write 展开为三个精确 Host 系统配置动作权限，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration069SettingsConfigEntryActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "settings.config.write";
    private const string ActionCreate = "settings.config.create";
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%069_SettingsConfigEntryActionPermissions.sql';
                  """
                : """
