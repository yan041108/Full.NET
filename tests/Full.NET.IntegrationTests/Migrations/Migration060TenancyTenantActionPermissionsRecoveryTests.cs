...
/// <summary>
/// 验证 060 将存量 tenancy.tenants.write 展开为四个精确 Host 租户动作权限，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration060TenancyTenantActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "tenancy.tenants.write";
    private const string ActionCreate = "tenancy.tenants.create";
        ...
        string apiKeyPermissionsJson)
    {
        await connection.ExecuteAsync(
            ...
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, N'Tenant Legacy', 0, 1,
                 0, @Now, NULL, 1);
            ...
            VALUES
                (@ApiKeyId, @UserId, N'Legacy Tenant Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
                RoleId = roleId,
                RoleCode = $"tenant-legacy-{roleId:N}"[..24],
                UserId = userId,
                Username = $"tlegacy-{userId:N}"[..24],
                NormalizedUsername = $"TLEGACY-{userId:N}"[..24],
        ...
        await connection.ExecuteAsync(
            ...
            INSERT INTO fn_identity_role
                (Id, TenantId, ScopeKey, Code, Name, IsSystem, IsActive,
                 IsSuperAdministrator, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, 'Tenant Legacy', 0, 1,
                 0, @Now, NULL, 1);
            ...
            INSERT INTO fn_identity_api_key
                (Id, UserId, DisplayName, KeyPrefix, KeyHash, PermissionsJson,
                 ...
                 CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@ApiKeyId, @UserId, 'Legacy Tenant Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%060_TenancyTenantActionPermissions.sql';
                  """
                : """
