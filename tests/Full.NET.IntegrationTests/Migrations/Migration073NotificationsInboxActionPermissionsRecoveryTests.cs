...
/// <summary>
/// 验证 073 将存量 notifications.inbox.write 展开为 send、为存量 notifications.inbox.read 补齐 mark_read/mark_all_read，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration073NotificationsInboxActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "notifications.inbox.write";
    private const string LegacyRead = "notifications.inbox.read";
    private const string ActionSend = "notifications.inbox.send";
    private const string ActionMarkAllRead = "notifications.inbox.mark_all_read";
    private const string ActionMarkRead = "notifications.inbox.mark_read";
    ...
    private static readonly string[] ReadExpandedPermissions =
    [
        LegacyRead,
        ActionMarkRead,
        ActionMarkAllRead,
    ...
    [TestMethod]
    public async Task SqlServer_expands_legacy_write_role_and_api_key_grants()
    {
        ...
        Assert.AreEqual(0, await CountLegacyWriteRolePermissionsAsync(connection, isSqlServer: true));
    ...
    [TestMethod]
    public async Task SqlServer_adds_mark_read_actions_to_legacy_read_grants()
    {
        var connectionString = await SharedDatabaseFixture.CreateSqlServerDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.SqlServer, connectionString);
        await runner.MigrateAsync();
        ...
        await SeedSqlServerStateAsync(
            ...
            apiKeyId,
            now,
            legacyPermission: LegacyRead,
            apiKeyPermissionsJson: $$"""["{{LegacyRead}}"]""");

        await DeleteMigrationRecordAsync(connection, isSqlServer: true);
        var recovered = await runner.MigrateAsync();

        Assert.AreEqual(1, recovered.ExecutedScriptCount);
        await AssertRolePermissionsAsync(connection, roleId, isSqlServer: true, ReadExpandedPermissions);
        await AssertApiKeyPermissionsAsync(connection, apiKeyId, isSqlServer: true, ReadExpandedPermissions);
        Assert.AreEqual(0, (await runner.MigrateAsync()).ExecutedScriptCount);
    }
    ...
    public async Task SqlServer_deduplicates_existing_action_grants()
    {
        ...
        await SeedSqlServerStateAsync(
            ...
            legacyPermission: LegacyWrite,
            apiKeyPermissionsJson:
                $$"""["{{LegacyWrite}}","{{ActionSend}}","{{UnrelatedPermission}}","{{LegacyWrite}}","{{ActionSend}}"]""");
    ...
    public async Task SqlServer_partial_expansion_converges_on_rerun()
    {
        ...
        await InsertRoleActionGrantsAsync(connection, roleId, isSqlServer: true, ActionSend);
    ...
    [TestMethod]
    public async Task MySql_expands_legacy_write_role_and_api_key_grants()
    {
        ...
        Assert.AreEqual(0, await CountLegacyWriteRolePermissionsAsync(connection, isSqlServer: false));
        await AssertApiKeyPermissionsAsync(
            connection,
            apiKeyId,
            isSqlServer: false,
    ...
    [TestMethod]
    public async Task MySql_adds_mark_read_actions_to_legacy_read_grants()
    {
        var connectionString = await SharedDatabaseFixture.CreateMySqlDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.MySql, connectionString);
        await runner.MigrateAsync();
        ...
        Assert.AreEqual(1, recovered.ExecutedScriptCount);
        await AssertRolePermissionsAsync(connection, roleId, isSqlServer: false, ReadExpandedPermissions);
        await AssertApiKeyPermissionsAsync(connection, apiKeyId, isSqlServer: false, ReadExpandedPermissions);
        Assert.AreEqual(0, (await runner.MigrateAsync()).ExecutedScriptCount);
    }
    ...
    public async Task MySql_partial_expansion_converges_on_rerun()
    {
        ...
        await SeedMySqlStateAsync(
            ...
            legacyPermission: LegacyWrite,
            apiKeyPermissionsJson: $$"""["{{LegacyWrite}}"]""");
        await InsertRoleActionGrantsAsync(connection, roleId, isSqlServer: false, ActionSend);
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%073_NotificationsInboxActionPermissions.sql';
                  """
                : """
    ...
    private static Task<int> CountLegacyWriteRolePermissionsAsync(
        System.Data.Common.DbConnection connection,
        bool isSqlServer)
    {
        var tableName = isSqlServer ? "dbo.fn_identity_role_permission" : "fn_identity_role_permission";
