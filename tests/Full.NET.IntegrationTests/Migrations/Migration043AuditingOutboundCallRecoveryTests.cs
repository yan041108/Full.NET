using Dapper;
using Full.NET.Data.Abstractions;
using Full.NET.Data.MySql;
using Full.NET.Migrations.DbUp;
using Microsoft.Data.SqlClient;
using Microsoft.Extensions.Logging.Abstractions;
using Microsoft.Extensions.Options;
using MySqlConnector;

namespace Full.NET.IntegrationTests.Migrations;

/// <summary>
/// 验证 043 在出站审计表已存在但索引缺失或形状错误时无损收敛。
/// </summary>
[TestClass]
public sealed class Migration043AuditingOutboundCallRecoveryTests
{
    private const string TimelineIndex =
        "IX_fn_auditing_outbound_call_OccurredAtUtc_Id";
    private const string ProviderIndex =
        "IX_fn_auditing_outbound_call_ProviderKey_OccurredAtUtc";

    [TestMethod]
    public async Task SqlServer_outbound_audit_migration_recovers_indexes_without_dropping_data()
    {
        var connectionString = await SharedDatabaseFixture.CreateSqlServerDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.SqlServer, connectionString);
        await runner.MigrateAsync();

        await using var connection = new SqlConnection(connectionString);
        await connection.ExecuteAsync(
            $"""
             INSERT INTO dbo.fn_auditing_outbound_call
                 (Id, OccurredAtUtc, ProviderKey, OperationKey,
                  DestinationHostCategory, StatusCode, Succeeded,
                  DurationMs, RetryCount)
             VALUES
                 (NEWID(), SYSDATETIMEOFFSET(), 'payments.stripe', 'charge.create',
                  'api.stripe.com', 200, 1, 12, 0);
             DROP INDEX {TimelineIndex} ON dbo.fn_auditing_outbound_call;
             DROP INDEX {ProviderIndex} ON dbo.fn_auditing_outbound_call;
             CREATE CLUSTERED INDEX {ProviderIndex}
                 ON dbo.fn_auditing_outbound_call (OccurredAtUtc);
             DELETE FROM dbo.SchemaVersions
             WHERE ScriptName LIKE '%043_AuditingOutboundCall.sql';
             """);

        var recovered = await runner.MigrateAsync();

        Assert.AreEqual(1, recovered.ExecutedScriptCount);
        Assert.AreEqual(1, await connection.ExecuteScalarAsync<int>(
            "SELECT COUNT(*) FROM dbo.fn_auditing_outbound_call"));
        await AssertSqlServerIndexAsync(
            connection,
            TimelineIndex,
            "OccurredAtUtc",
            "Id",
            clustered: true);
        await AssertSqlServerIndexAsync(
            connection,
            ProviderIndex,
            "ProviderKey",
            "OccurredAtUtc",
            clustered: false);
        Assert.AreEqual(0, (await runner.MigrateAsync()).ExecutedScriptCount);
    }

    [TestMethod]
    public async Task MySql_outbound_audit_migration_recovers_indexes_without_dropping_data()
    {
        var connectionString = await SharedDatabaseFixture.CreateMySqlDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.MySql, connectionString);
        await runner.MigrateAsync();

        await using var connection = new MySqlConnection(
            MySqlConnectionStringPolicy.Create(
                connectionString,
                MySqlGuidStorageMode.Binary16,
                allowUserVariables: false));
        await connection.ExecuteAsync(
            $"""
             INSERT INTO fn_auditing_outbound_call
                 (Id, OccurredAtUtc, ProviderKey, OperationKey,
                  DestinationHostCategory, StatusCode, Succeeded,
                  DurationMs, RetryCount)
             VALUES
                 (UNHEX(REPLACE(UUID(), '-', '')), UTC_TIMESTAMP(6),
                  'payments.stripe', 'charge.create', 'api.stripe.com',
                  200, 1, 12, 0);
             DROP INDEX {TimelineIndex} ON fn_auditing_outbound_call;
             DROP INDEX {ProviderIndex} ON fn_auditing_outbound_call;
             CREATE INDEX {ProviderIndex}
                 ON fn_auditing_outbound_call (OccurredAtUtc);
             DELETE FROM schemaversions
             WHERE ScriptName LIKE '%043_AuditingOutboundCall.sql';
             """);

        var recovered = await runner.MigrateAsync();

        Assert.AreEqual(1, recovered.ExecutedScriptCount);
        Assert.AreEqual(1, await connection.ExecuteScalarAsync<int>(
            "SELECT COUNT(*) FROM fn_auditing_outbound_call"));
        await AssertMySqlIndexAsync(
            connection,
            TimelineIndex,
            "OccurredAtUtc",
            "Id");
        await AssertMySqlIndexAsync(
            connection,
            ProviderIndex,
            "ProviderKey",
            "OccurredAtUtc");
        Assert.AreEqual(0, (await runner.MigrateAsync()).ExecutedScriptCount);
    }

    private static async Task AssertSqlServerIndexAsync(
        SqlConnection connection,
        string indexName,
        string firstColumn,
        string secondColumn,
        bool clustered)
    {
        Assert.AreEqual(2, await connection.ExecuteScalarAsync<int>(
            """
            SELECT COUNT(*)
            FROM sys.indexes AS indexObject
            INNER JOIN sys.index_columns AS indexColumn
                ON indexColumn.object_id = indexObject.object_id
               AND indexColumn.index_id = indexObject.index_id
            INNER JOIN sys.columns AS columnObject
                ON columnObject.object_id = indexColumn.object_id
               AND columnObject.column_id = indexColumn.column_id
            WHERE indexObject.object_id = OBJECT_ID(N'dbo.fn_auditing_outbound_call')
              AND indexObject.name = @IndexName
              AND indexObject.is_unique = 0
              AND indexObject.type = @IndexType
              AND ((indexColumn.key_ordinal = 1 AND columnObject.name = @FirstColumn)
                   OR (indexColumn.key_ordinal = 2 AND columnObject.name = @SecondColumn))
            """,
            new
            {
                IndexName = indexName,
                IndexType = clustered ? 1 : 2,
                FirstColumn = firstColumn,
                SecondColumn = secondColumn,
            }));
    }

    private static async Task AssertMySqlIndexAsync(
        MySqlConnection connection,
        string indexName,
        string firstColumn,
        string secondColumn)
    {
        Assert.AreEqual(2, await connection.ExecuteScalarAsync<int>(
            """
            SELECT COUNT(*)
            FROM INFORMATION_SCHEMA.STATISTICS
            WHERE TABLE_SCHEMA = DATABASE()
              AND TABLE_NAME = 'fn_auditing_outbound_call'
              AND INDEX_NAME = @IndexName
              AND NON_UNIQUE = 1
              AND SUB_PART IS NULL
              AND ((SEQ_IN_INDEX = 1 AND COLUMN_NAME = @FirstColumn)
                   OR (SEQ_IN_INDEX = 2 AND COLUMN_NAME = @SecondColumn))
            """,
            new
            {
                IndexName = indexName,
                FirstColumn = firstColumn,
                SecondColumn = secondColumn,
            }));
    }

    private static DbUpMigrationRunner CreateRunner(
        DatabaseProvider provider,
        string connectionString) =>
        new(
            Options.Create(new DatabaseOptions
            {
                Provider = provider,
                ConnectionString = connectionString,
                MySqlGuidStorageMode = MySqlGuidStorageMode.Binary16,
                CommandTimeoutSeconds = 300,
            }),
            NullLoggerFactory.Instance,
            MigrationContractOptionFactory.UuidOptions(),
            MigrationContractOptionFactory.NamingOptions());
}
