...
/// <summary>
/// 验证 061 将存量 tenancy.tenant_packages.write 展开为三个精确 Host 租户套餐动作权限，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration061TenancyTenantPackageActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "tenancy.tenant_packages.write";
    private const string ActionCreate = "tenancy.tenant_packages.create";
        ...
        string apiKeyPermissionsJson)
    {
        await connection.ExecuteAsync(
            ...
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, N'Package Legacy', 0, 1,
                 0, @Now, NULL, 1);
            ...
            new
            {
                ...
                UserId = userId,
                Username = $"plegacy-{userId:N}"[..24],
                NormalizedUsername = $"plegacy-{userId:N}"[..24],
        ...
        await connection.ExecuteAsync(
            ...
            INSERT INTO fn_identity_role
                (Id, TenantId, ScopeKey, Code, Name, IsSystem, IsActive,
                 IsSuperAdministrator, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, 'Package Legacy', 0, 1,
                 0, @Now, NULL, 1);
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%061_TenancyTenantPackageActionPermissions.sql';
                  """
                : """
