...
/// <summary>
/// 验证 078 将存量 document.host_documents.write 展开为三个精确 Host 文档动作权限，并为存量 delete 补齐 restore，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration078DocumentItemActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "document.host_documents.write";
    private const string ActionCreate = "document.host_documents.create";
    private const string UnrelatedPermission = "identity.roles.read";

    private const string LegacyDelete = "document.host_documents.delete";
    private const string ActionRestore = "document.host_documents.restore";
    ...
    private static readonly string[] DeleteExpandedPermissions =
    [
        LegacyDelete,
        ActionRestore,
    ];
    ...
    [TestMethod]
    public async Task SqlServer_adds_restore_to_legacy_delete_grants()
    {
        var connectionString = await SharedDatabaseFixture.CreateSqlServerDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.SqlServer, connectionString);
        await runner.MigrateAsync();
        ...
        await SeedSqlServerStateAsync(
            ...
            now,
            insertExistingActionGrants: false,
            legacyPermission: LegacyDelete,
            apiKeyPermissionsJson: $$"""["{{LegacyDelete}}"]""");

        await DeleteMigrationRecordAsync(connection, isSqlServer: true);
        var recovered = await runner.MigrateAsync();

        Assert.AreEqual(1, recovered.ExecutedScriptCount);
        await AssertRolePermissionsAsync(connection, roleId, isSqlServer: true, DeleteExpandedPermissions);
        await AssertApiKeyPermissionsAsync(connection, apiKeyId, isSqlServer: true, DeleteExpandedPermissions);
        Assert.AreEqual(0, (await runner.MigrateAsync()).ExecutedScriptCount);
    }

    [TestMethod]
    public async Task MySql_adds_restore_to_legacy_delete_grants()
    {
        var connectionString = await SharedDatabaseFixture.CreateMySqlDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.MySql, connectionString);
        await runner.MigrateAsync();
        ...
        Assert.AreEqual(1, recovered.ExecutedScriptCount);
        await AssertRolePermissionsAsync(connection, roleId, isSqlServer: false, DeleteExpandedPermissions);
        await AssertApiKeyPermissionsAsync(connection, apiKeyId, isSqlServer: false, DeleteExpandedPermissions);
        Assert.AreEqual(0, (await runner.MigrateAsync()).ExecutedScriptCount);
    }
    ...
    private static async Task SeedSqlServerStateAsync(
        ...
        string legacyPermission = LegacyWrite)
    {
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%078_DocumentItemActionPermissions.sql';
                  """
                : """
