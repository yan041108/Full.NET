...
/// <summary>
/// 验证 079 将存量 document.categories.manage 展开为四个精确 Host 文档分类动作权限，并为存量 document.host_documents.read 补齐 categories.read，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration079DocumentCategoryActionPermissionsRecoveryTests
{
    private const string LegacyManage = "document.categories.manage";
    private const string ActionCreate = "document.categories.create";
    ...
    private const string ActionCategoryRead = "document.categories.read";
    ...
    [TestMethod]
    public async Task SqlServer_adds_category_read_to_legacy_host_document_read_grants()
    {
        var connectionString = await SharedDatabaseFixture.CreateSqlServerDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.SqlServer, connectionString);
        await runner.MigrateAsync();
    ...
    [TestMethod]
    public async Task MySql_adds_category_read_to_legacy_host_document_read_grants()
    {
        var connectionString = await SharedDatabaseFixture.CreateMySqlDatabaseAsync();
        var runner = CreateRunner(DatabaseProvider.MySql, connectionString);
        await runner.MigrateAsync();
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%079_DocumentCategoryActionPermissions.sql';
                  """
                : """
