...
/// <summary>
/// 验证 054 将存量 identity.users.write 展开为六个精确 Host 用户动作权限，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration054IdentityUserActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "identity.users.write";
    private const string ActionCreate = "identity.users.create";
    private const string ActionUpdate = "identity.users.update";
    private const string ActionAssignRoles = "identity.users.assign_roles";
    private const string ActionResetPassword = "identity.users.reset_password";
    private const string ActionDisable = "identity.users.disable";
    private const string ActionEnable = "identity.users.enable";
        ...
        string apiKeyPermissionsJson)
    {
        await connection.ExecuteAsync(
            ...
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, N'User Action Legacy', 0, 1,
                 0, @Now, NULL, 1);
            ...
            VALUES
                (@ApiKeyId, @UserId, N'Legacy User Action Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
                RoleId = roleId,
                RoleCode = $"user-action-{roleId:N}"[..24],
                UserId = userId,
                Username = $"uact-{userId:N}"[..24],
                NormalizedUsername = $"UACT-{userId:N}"[..24],
        ...
        await connection.ExecuteAsync(
            ...
            INSERT INTO fn_identity_role
                (Id, TenantId, ScopeKey, Code, Name, IsSystem, IsActive,
                 IsSuperAdministrator, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, 'User Action Legacy', 0, 1,
                 0, @Now, NULL, 1);
            ...
            INSERT INTO fn_identity_api_key
                (Id, UserId, DisplayName, KeyPrefix, KeyHash, PermissionsJson,
                 ...
                 CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@ApiKeyId, @UserId, 'Legacy User Action Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%054_IdentityUserActionPermissions.sql';
                  """
                : """
