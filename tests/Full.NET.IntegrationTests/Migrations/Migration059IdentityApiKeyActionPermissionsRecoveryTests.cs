...
/// <summary>
/// 验证 059 将存量 identity.api_keys.write 展开为三个精确 Host API Key 动作权限，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration059IdentityApiKeyActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "identity.api_keys.write";
    private const string ActionCreate = "identity.api_keys.create";
        ...
        string apiKeyPermissionsJson)
    {
        await connection.ExecuteAsync(
            ...
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, N'API Key Legacy', 0, 1,
                 0, @Now, NULL, 1);
            ...
            VALUES
                (@UserId, NULL, 'host', @Username, @NormalizedUsername, N'API Key User',
            ...
            VALUES
                (@ApiKeyId, @UserId, N'Legacy API Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
                RoleId = roleId,
                RoleCode = $"apikey-legacy-{roleId:N}"[..24],
                UserId = userId,
                Username = $"akegacy-{userId:N}"[..24],
                NormalizedUsername = $"ALEGACY-{userId:N}"[..24],
        ...
        await connection.ExecuteAsync(
            ...
            INSERT INTO fn_identity_role
                (Id, TenantId, ScopeKey, Code, Name, IsSystem, IsActive,
                 IsSuperAdministrator, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, 'API Key Legacy', 0, 1,
                 0, @Now, NULL, 1);
            INSERT INTO fn_identity_user
                (Id, TenantId, ScopeKey, Username, NormalizedUsername, DisplayName,
                 ...
                 SecurityStamp, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@UserId, NULL, 'host', @Username, @NormalizedUsername, 'API Key User',
                 'unused', 1, 0, NULL, @SecurityStamp, @Now, NULL, 1);
            ...
            INSERT INTO fn_identity_api_key
                (Id, UserId, DisplayName, KeyPrefix, KeyHash, PermissionsJson,
                 ...
                 CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@ApiKeyId, @UserId, 'Legacy API Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%059_IdentityApiKeyActionPermissions.sql';
                  """
                : """
