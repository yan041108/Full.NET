...
/// <summary>
/// 验证 056 将存量 identity.role_field_grants.write 展开为 identity.role_field_grants.replace，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration056IdentityRoleFieldGrantActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "identity.role_field_grants.write";
    private const string ActionReplace = "identity.role_field_grants.replace";
    ...
    public async Task SqlServer_deduplicates_existing_action_grants()
    {
        ...
        await SeedSqlServerStateAsync(
            ...
            insertExistingActionGrants: false,
            apiKeyPermissionsJson:
                $$"""["{{LegacyWrite}}","{{ActionReplace}}","{{UnrelatedPermission}}","{{LegacyWrite}}","{{ActionReplace}}"]""");
    ...
    public async Task SqlServer_partial_expansion_converges_on_rerun()
    {
        ...
        await InsertRoleActionGrantsAsync(connection, roleId, isSqlServer: true, ActionReplace);
    ...
    public async Task MySql_partial_expansion_converges_on_rerun()
    {
        ...
        await SeedMySqlStateAsync(
            ...
            insertExistingActionGrants: false,
            apiKeyPermissionsJson: $$"""["{{LegacyWrite}}"]""");
        await InsertRoleActionGrantsAsync(connection, roleId, isSqlServer: false, ActionReplace);
        ...
        string apiKeyPermissionsJson)
    {
        await connection.ExecuteAsync(
            ...
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, N'Field Grant Legacy', 0, 1,
                 0, @Now, NULL, 1);
            ...
            VALUES
                (@UserId, NULL, 'host', @Username, @NormalizedUsername, N'API Field Grant User',
            ...
            VALUES
                (@ApiKeyId, @UserId, N'Legacy Field Grant Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
                RoleId = roleId,
                RoleCode = $"field-grant-{roleId:N}"[..24],
                UserId = userId,
                Username = $"fgrant-{userId:N}"[..24],
                NormalizedUsername = $"FGRANT-{userId:N}"[..24],
        ...
        if (insertExistingActionGrants)
        {
            await connection.ExecuteAsync(
                """
                INSERT INTO dbo.fn_identity_role_permission (RoleId, PermissionCode)
                VALUES (@RoleId, @ActionReplace);
                """,
                new { RoleId = roleId, ActionReplace });
        }
    }
        ...
        await connection.ExecuteAsync(
            ...
            INSERT INTO fn_identity_role
                (Id, TenantId, ScopeKey, Code, Name, IsSystem, IsActive,
                 IsSuperAdministrator, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@RoleId, NULL, 'host', @RoleCode, 'Field Grant Legacy', 0, 1,
                 0, @Now, NULL, 1);
            INSERT INTO fn_identity_user
                (Id, TenantId, ScopeKey, Username, NormalizedUsername, DisplayName,
                 ...
                 SecurityStamp, CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@UserId, NULL, 'host', @Username, @NormalizedUsername, 'API Field Grant User',
                 'unused', 1, 0, NULL, @SecurityStamp, @Now, NULL, 1);
            ...
            INSERT INTO fn_identity_api_key
                (Id, UserId, DisplayName, KeyPrefix, KeyHash, PermissionsJson,
                 ...
                 CreatedAtUtc, UpdatedAtUtc, Version)
            VALUES
                (@ApiKeyId, @UserId, 'Legacy Field Grant Key', @KeyPrefix, @KeyHash,
                 @PermissionsJson, NULL, 1, NULL, NULL, @Now, NULL, 1);
            """,
            new
            {
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%056_IdentityRoleFieldGrantActionPermissions.sql';
                  """
                : """
