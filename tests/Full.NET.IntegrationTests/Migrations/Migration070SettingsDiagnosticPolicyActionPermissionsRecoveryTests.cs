...
/// <summary>
/// 验证 070 将存量 settings.diagnostic_policy.write 展开为两个精确 Host 限时诊断策略动作权限，并安全处理角色重复授权与 API Key JSON 去重。
/// </summary>
[TestClass]
public sealed class Migration070SettingsDiagnosticPolicyActionPermissionsRecoveryTests
{
    private const string LegacyWrite = "settings.diagnostic_policy.write";
    private const string ActionUpdate = "settings.diagnostic_policy.update";
    ...
    public async Task SqlServer_deduplicates_existing_action_grants()
    {
        ...
        await SeedSqlServerStateAsync(
            ...
            insertExistingActionGrants: false,
            apiKeyPermissionsJson:
                $$"""["{{LegacyWrite}}","{{ActionUpdate}}","{{UnrelatedPermission}}","{{LegacyWrite}}","{{ActionUpdate}}"]""");
    ...
    public async Task SqlServer_partial_expansion_converges_on_rerun()
    {
        ...
        await InsertRoleActionGrantsAsync(connection, roleId, isSqlServer: true, ActionUpdate);
    ...
    public async Task MySql_partial_expansion_converges_on_rerun()
    {
        ...
        await SeedMySqlStateAsync(
            ...
            insertExistingActionGrants: false,
            apiKeyPermissionsJson: $$"""["{{LegacyWrite}}"]""");
        await InsertRoleActionGrantsAsync(connection, roleId, isSqlServer: false, ActionUpdate);
        ...
        string apiKeyPermissionsJson)
    {
        ...
        if (insertExistingActionGrants)
        {
            await connection.ExecuteAsync(
                """
                INSERT INTO dbo.fn_identity_role_permission (RoleId, PermissionCode)
                VALUES (@RoleId, @ActionUpdate);
                """,
                new { RoleId = roleId, ActionUpdate });
        }
    }
        ...
        connection.ExecuteAsync(
            isSqlServer
                ? """
                  ...
                  WHERE ScriptName LIKE '%070_SettingsDiagnosticPolicyActionPermissions.sql';
                  """
                : """
