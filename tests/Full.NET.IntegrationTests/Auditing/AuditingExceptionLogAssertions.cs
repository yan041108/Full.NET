...
internal static class AuditingExceptionLogAssertions
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await VerifyProbeWriteAndQueryAsync(client, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        Assert.IsTrue(page.Items.Any(item =>
            string.Equals(item.Message, "auditing.exception_probe", StringComparison.Ordinal)
            && item.RequestPath is not null
            && item.RequestPath.Contains(
                "/api/v1/auditing/exception-probes",
