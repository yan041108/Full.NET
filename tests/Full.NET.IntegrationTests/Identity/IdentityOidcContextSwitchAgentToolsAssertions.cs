...
internal static class IdentityOidcContextSwitchAgentToolsAssertions
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await VerifyAgentToolsTracksContextSwitchAsync(factory, cancellationToken);
    }

    private static async Task VerifyAgentToolsTracksContextSwitchAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var flow = await IdentityOidcRelyingPartyFixture.RunAuthorizationCodeFlowAsync(
            ...
            cancellationToken: cancellationToken);
        await AssertAgentToolsAcceptsTokenAsync(
            hostClient,
            flow.AccessToken,
        ...
        await AssertMeAcceptsTokenAsync(
            ...
            cancellationToken);
        await AssertAgentToolsRejectsTokenAsync(
            hostClient,
            flow.AccessToken,
    ...
    private static async Task AssertAgentToolsAcceptsTokenAsync(
        HttpClient client,
        string accessToken,
        string scenario,
        CancellationToken cancellationToken)
    {
        using var request = new HttpRequestMessage(HttpMethod.Get, "/api/v1/ai/agent-tools");
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", accessToken);
        using var response = await client.SendAsync(request, cancellationToken);
        Assert.AreEqual(
            HttpStatusCode.OK,
            response.StatusCode,
            $"Access token must reach AI tool catalog for {scenario}.");
    }

    private static async Task AssertAgentToolsRejectsTokenAsync(
        HttpClient client,
        string accessToken,
        string scenario,
        CancellationToken cancellationToken)
