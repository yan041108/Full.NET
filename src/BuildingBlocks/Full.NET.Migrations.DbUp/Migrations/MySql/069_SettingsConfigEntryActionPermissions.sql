...
INNER JOIN (
    SELECT 'settings.config.create' AS PermissionCode
    UNION ALL SELECT 'settings.config.update'
    UNION ALL SELECT 'settings.config.disable'
) AS actions
WHERE legacy.PermissionCode = 'settings.config.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'settings.config.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'settings.config.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%settings.config.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%settings.config.write%';