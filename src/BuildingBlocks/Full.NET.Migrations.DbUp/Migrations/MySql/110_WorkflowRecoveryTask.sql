...
CREATE TABLE IF NOT EXISTS fn_workflow_recovery_task (
    ...
    OpenOccupancyKey varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin GENERATED ALWAYS AS (CASE WHEN StatusKey IN ('pending', 'failed', 'dead_lettered') THEN CONCAT(TenantScopeKey, '|', HEX(InstanceId), '|', KindKey, '|', IFNULL(HEX(StepId), 'NONE')) ELSE NULL END) STORED COMMENT '未关闭恢复任务占用键',
