...
INNER JOIN (
    SELECT 'settings.tenant_dict_types.create' AS PermissionCode
    UNION ALL SELECT 'settings.tenant_dict_types.update'
    UNION ALL SELECT 'settings.tenant_dict_types.disable'
) AS actions
WHERE legacy.PermissionCode = 'settings.tenant_dict_types.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'settings.tenant_dict_types.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'settings.tenant_dict_types.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%settings.tenant_dict_types.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%settings.tenant_dict_types.write%';