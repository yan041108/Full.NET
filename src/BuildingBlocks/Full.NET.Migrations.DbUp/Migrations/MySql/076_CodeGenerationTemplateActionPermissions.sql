...
INNER JOIN (
    SELECT 'codegen.templates.create' AS PermissionCode
    UNION ALL SELECT 'codegen.templates.update'
    UNION ALL SELECT 'codegen.templates.delete'
    
) AS actions
WHERE legacy.PermissionCode = 'codegen.templates.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'codegen.templates.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'codegen.templates.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%codegen.templates.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%codegen.templates.write%';