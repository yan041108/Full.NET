...
INNER JOIN (
    SELECT 'organization.units.create' AS PermissionCode
    UNION ALL SELECT 'organization.units.update'
    UNION ALL SELECT 'organization.units.disable'
) AS actions
WHERE legacy.PermissionCode = 'organization.units.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'organization.units.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'organization.units.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%organization.units.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%organization.units.write%';