...
INNER JOIN (
    SELECT 'tenancy.tenants.create' AS PermissionCode
    UNION ALL SELECT 'tenancy.tenants.update'
    UNION ALL SELECT 'tenancy.tenants.disable'
    UNION ALL SELECT 'tenancy.tenants.assign_package'
) AS actions
WHERE legacy.PermissionCode = 'tenancy.tenants.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'tenancy.tenants.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'tenancy.tenants.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%tenancy.tenants.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%tenancy.tenants.write%';