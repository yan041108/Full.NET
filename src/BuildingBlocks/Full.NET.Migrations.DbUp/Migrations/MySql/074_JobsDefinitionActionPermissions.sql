-- 074：将存量 jobs.definitions.write 展开为 create/update/disable/trigger 四个精确动作权限，幂等处理角色行与 API Key JSON。
INSERT INTO fn_identity_role_permission (RoleId, PermissionCode)
SELECT legacy.RoleId, actions.PermissionCode
FROM fn_identity_role_permission AS legacy
INNER JOIN (
    SELECT 'jobs.definitions.create' AS PermissionCode
    UNION ALL SELECT 'jobs.definitions.update'
    UNION ALL SELECT 'jobs.definitions.disable'
    UNION ALL SELECT 'jobs.definitions.trigger'
) AS actions
WHERE legacy.PermissionCode = 'jobs.definitions.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'jobs.definitions.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'jobs.definitions.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%jobs.definitions.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%jobs.definitions.write%';