-- 055：将存量 identity.roles.write 展开为五个精确 Host 角色动作权限，幂等处理角色行与 API Key JSON。

INSERT INTO fn_identity_role_permission (RoleId, PermissionCode)
SELECT legacy.RoleId, actions.PermissionCode
FROM fn_identity_role_permission AS legacy
INNER JOIN (
    SELECT 'identity.roles.create' AS PermissionCode
    UNION ALL SELECT 'identity.roles.update'
    UNION ALL SELECT 'identity.roles.assign_permissions'
    UNION ALL SELECT 'identity.roles.disable'
    UNION ALL SELECT 'identity.roles.assign_data_scope'
) AS actions
WHERE legacy.PermissionCode = 'identity.roles.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'identity.roles.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'identity.roles.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%identity.roles.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%identity.roles.write%';