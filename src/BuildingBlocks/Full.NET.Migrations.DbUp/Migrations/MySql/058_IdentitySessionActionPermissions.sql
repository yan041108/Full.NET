-- 058：将存量 identity.sessions.write 展开为 identity.sessions.revoke，幂等处理角色行与 API Key JSON。

INSERT INTO fn_identity_role_permission (RoleId, PermissionCode)
SELECT legacy.RoleId, actions.PermissionCode
FROM fn_identity_role_permission AS legacy
INNER JOIN (
    SELECT 'identity.sessions.revoke' AS PermissionCode
) AS actions
WHERE legacy.PermissionCode = 'identity.sessions.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'identity.sessions.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'identity.sessions.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%identity.sessions.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%identity.sessions.write%';