-- 067：将存量 settings.dict_types.write 展开为三个精确 Host 数据字典动作权限，幂等处理角色行与 API Key JSON。
INSERT INTO fn_identity_role_permission (RoleId, PermissionCode)
SELECT legacy.RoleId, actions.PermissionCode
FROM fn_identity_role_permission AS legacy
INNER JOIN (
    SELECT 'settings.dict_types.create' AS PermissionCode
    UNION ALL SELECT 'settings.dict_types.update'
    UNION ALL SELECT 'settings.dict_types.disable'
) AS actions
WHERE legacy.PermissionCode = 'settings.dict_types.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'settings.dict_types.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'settings.dict_types.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%settings.dict_types.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%settings.dict_types.write%';