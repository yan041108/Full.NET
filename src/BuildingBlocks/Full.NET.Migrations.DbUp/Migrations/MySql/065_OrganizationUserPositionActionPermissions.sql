-- 065：将存量 organization.user_positions.write 展开为三个精确租户用户职位隶属动作权限，幂等处理角色行与 API Key JSON。
INSERT INTO fn_identity_role_permission (RoleId, PermissionCode)
SELECT legacy.RoleId, actions.PermissionCode
FROM fn_identity_role_permission AS legacy
INNER JOIN (
    SELECT 'organization.user_positions.create' AS PermissionCode
    UNION ALL SELECT 'organization.user_positions.update'
    UNION ALL SELECT 'organization.user_positions.disable'
) AS actions
WHERE legacy.PermissionCode = 'organization.user_positions.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'organization.user_positions.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'organization.user_positions.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%organization.user_positions.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%organization.user_positions.write%';