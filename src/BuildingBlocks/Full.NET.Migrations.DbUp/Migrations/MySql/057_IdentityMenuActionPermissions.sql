...
INNER JOIN (
    SELECT 'identity.menus.create' AS PermissionCode
    UNION ALL SELECT 'identity.menus.update'
    UNION ALL SELECT 'identity.menus.disable'
) AS actions
WHERE legacy.PermissionCode = 'identity.menus.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'identity.menus.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'identity.menus.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%identity.menus.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%identity.menus.write%';