...
SELECT statisticsRole.RoleId, 'document.host_access_logs.read'
FROM fn_identity_role_permission AS statisticsRole
WHERE statisticsRole.PermissionCode = 'document.host_statistics.read'
  AND NOT EXISTS (
    SELECT 1
    FROM fn_identity_role_permission AS existing
    WHERE existing.RoleId = statisticsRole.RoleId
      AND existing.PermissionCode = 'document.host_access_logs.read'
  );
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            SELECT
                expanded.Id,
                'document.host_access_logs.read' AS elem
            FROM (
                ...
                WHERE elements.raw = 'document.host_statistics.read'
            ) AS expanded
        ) AS distinctMapped
        GROUP BY distinctMapped.Id, distinctMapped.elem
    ) AS mapped
        ON mapped.Id = source.Id
    WHERE source.PermissionsJson LIKE '%document.host_statistics.read%'
      AND source.PermissionsJson NOT LIKE '%document.host_access_logs.read%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%document.host_statistics.read%'
  AND apiKey.PermissionsJson NOT LIKE '%document.host_access_logs.read%';
