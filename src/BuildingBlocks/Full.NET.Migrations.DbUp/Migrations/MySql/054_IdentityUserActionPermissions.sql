-- 054：将存量 identity.users.write 展开为六个精确 Host 用户动作权限，幂等处理角色行与 API Key JSON。

INSERT INTO fn_identity_role_permission (RoleId, PermissionCode)
SELECT legacy.RoleId, actions.PermissionCode
FROM fn_identity_role_permission AS legacy
INNER JOIN (
    SELECT 'identity.users.create' AS PermissionCode
    UNION ALL SELECT 'identity.users.update'
    UNION ALL SELECT 'identity.users.assign_roles'
    UNION ALL SELECT 'identity.users.reset_password'
    UNION ALL SELECT 'identity.users.disable'
    UNION ALL SELECT 'identity.users.enable'
) AS actions
WHERE legacy.PermissionCode = 'identity.users.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'identity.users.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'identity.users.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%identity.users.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%identity.users.write%';