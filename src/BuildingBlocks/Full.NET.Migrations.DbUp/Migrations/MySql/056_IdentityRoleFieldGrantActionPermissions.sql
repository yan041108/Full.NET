-- 056：将存量 identity.role_field_grants.write 展开为 identity.role_field_grants.replace，幂等处理角色行与 API Key JSON。

INSERT INTO fn_identity_role_permission (RoleId, PermissionCode)
SELECT legacy.RoleId, actions.PermissionCode
FROM fn_identity_role_permission AS legacy
INNER JOIN (
    SELECT 'identity.role_field_grants.replace' AS PermissionCode
) AS actions
WHERE legacy.PermissionCode = 'identity.role_field_grants.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = 'identity.role_field_grants.write';
...
INNER JOIN (
    ...
    INNER JOIN (
        ...
        FROM (
            ...
            FROM (
                ...
                WHERE elements.raw <> 'identity.role_field_grants.write'
            ) AS preserved
            UNION ALL
            SELECT
                expanded.Id,
    ...
    WHERE source.PermissionsJson LIKE '%identity.role_field_grants.write%'
    GROUP BY source.Id
) AS rebuilt ON rebuilt.Id = apiKey.Id
SET apiKey.PermissionsJson = rebuilt.PermissionsJson
WHERE apiKey.PermissionsJson LIKE '%identity.role_field_grants.write%';