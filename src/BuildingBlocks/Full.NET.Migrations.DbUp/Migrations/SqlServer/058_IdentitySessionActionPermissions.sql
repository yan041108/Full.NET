...
CROSS JOIN (
    VALUES (N'identity.sessions.revoke')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'identity.sessions.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'identity.sessions.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'identity.sessions.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'identity.sessions.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%identity.sessions.write%';