-- 059：将存量 identity.api_keys.write 展开为三个精确 Host API Key 动作权限，幂等处理角色行与 API Key JSON。
...
CROSS JOIN (
    VALUES
        (N'identity.api_keys.create'),
        (N'identity.api_keys.disable'),
        (N'identity.api_keys.rotate')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'identity.api_keys.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'identity.api_keys.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'identity.api_keys.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'identity.api_keys.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%identity.api_keys.write%';