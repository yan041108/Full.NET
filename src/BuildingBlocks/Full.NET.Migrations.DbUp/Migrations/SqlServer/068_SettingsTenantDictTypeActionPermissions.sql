-- 068：将存量 settings.tenant_dict_types.write 展开为三个精确 租户数据字典动作权限，幂等处理角色行与 API Key JSON。
...
CROSS JOIN (
    VALUES
        (N'settings.tenant_dict_types.create'),
        (N'settings.tenant_dict_types.update'),
        (N'settings.tenant_dict_types.disable')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'settings.tenant_dict_types.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'settings.tenant_dict_types.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'settings.tenant_dict_types.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'settings.tenant_dict_types.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%settings.tenant_dict_types.write%';