-- 063：将存量 organization.positions.write 展开为五个精确租户职位动作权限，幂等处理角色行与 API Key JSON。
...
CROSS JOIN (
    VALUES
        (N'organization.positions.create'),
        (N'organization.positions.update'),
        (N'organization.positions.disable'),
        (N'organization.positions.assign_unit'),
        (N'organization.positions.assign_position_level')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'organization.positions.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'organization.positions.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'organization.positions.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'organization.positions.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%organization.positions.write%';