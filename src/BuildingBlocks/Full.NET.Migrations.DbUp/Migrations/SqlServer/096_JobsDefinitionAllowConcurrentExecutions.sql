...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        EXEC sys.sp_addextendedproperty @name=N'MS_Description', @value=N'是否允许并发执行', @level0type=N'SCHEMA', @level0name=N'dbo', @level1type=N'TABLE', @level1name=N'fn_jobs_definition', @level2type=N'COLUMN', @level2name=N'AllowConcurrentExecutions';
    IF NOT EXISTS (
        SELECT 1
        FROM sys.extended_properties
        WHERE class = 1
