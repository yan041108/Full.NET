-- 062：将存量 organization.units.write 展开为三个精确租户机构动作权限，幂等处理角色行与 API Key JSON。
...
CROSS JOIN (
    VALUES
        (N'organization.units.create'),
        (N'organization.units.update'),
        (N'organization.units.disable')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'organization.units.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'organization.units.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'organization.units.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'organization.units.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%organization.units.write%';