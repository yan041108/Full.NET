...
CROSS JOIN (
    VALUES (N'identity.role_field_grants.replace')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'identity.role_field_grants.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'identity.role_field_grants.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'identity.role_field_grants.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'identity.role_field_grants.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%identity.role_field_grants.write%';