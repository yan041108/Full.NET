...
CROSS JOIN (
    VALUES
        (N'settings.dict_types.create'),
        (N'settings.dict_types.update'),
        (N'settings.dict_types.disable')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'settings.dict_types.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'settings.dict_types.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'settings.dict_types.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'settings.dict_types.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%settings.dict_types.write%';