...
CROSS JOIN (
    VALUES
        (N'organization.user_positions.create'),
        (N'organization.user_positions.update'),
        (N'organization.user_positions.disable')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'organization.user_positions.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'organization.user_positions.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'organization.user_positions.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'organization.user_positions.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%organization.user_positions.write%';