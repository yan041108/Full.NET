-- 070：将存量 settings.diagnostic_policy.write 展开为两个精确 Host 限时诊断策略动作权限，幂等处理角色行与 API Key JSON。
...
CROSS JOIN (
    VALUES
        (N'settings.diagnostic_policy.update'),
        (N'settings.diagnostic_policy.restore')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'settings.diagnostic_policy.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'settings.diagnostic_policy.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'settings.diagnostic_policy.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'settings.diagnostic_policy.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%settings.diagnostic_policy.write%';
