...
CROSS JOIN (
    VALUES
        (N'jobs.definitions.create'),
        (N'jobs.definitions.update'),
        (N'jobs.definitions.disable'),
        (N'jobs.definitions.trigger')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'jobs.definitions.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'jobs.definitions.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'jobs.definitions.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'jobs.definitions.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%jobs.definitions.write%';