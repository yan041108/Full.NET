...
CROSS JOIN (
    VALUES
        (N'identity.roles.create'),
        (N'identity.roles.update'),
        (N'identity.roles.assign_permissions'),
        (N'identity.roles.disable'),
        (N'identity.roles.assign_data_scope')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'identity.roles.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'identity.roles.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'identity.roles.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'identity.roles.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%identity.roles.write%';