...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE class = 1
          ...
          AND minor_id = COLUMNPROPERTY(OBJECT_ID(N'dbo.fn_identity_open_access_client'), N'DailyRequestQuota', 'ColumnId')

          AND name = N'MS_Description'

    )

        EXEC sys.sp_addextendedproperty @name=N'MS_Description', @value=N'每日请求配额', @level0type=N'SCHEMA', @level0name=N'dbo', @level1type=N'TABLE', @level1name=N'fn_identity_open_access_client', @level2type=N'COLUMN', @level2name=N'DailyRequestQuota';
