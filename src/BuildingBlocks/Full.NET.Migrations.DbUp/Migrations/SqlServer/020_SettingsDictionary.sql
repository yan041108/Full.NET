...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        EXEC sys.sp_addextendedproperty @name=N'MS_Description', @value=N'字典类型标识', @level0type=N'SCHEMA', @level0name=N'dbo', @level1type=N'TABLE', @level1name=N'fn_settings_dict_item', @level2type=N'COLUMN', @level2name=N'DictTypeId';
    IF NOT EXISTS (
        SELECT 1
        FROM sys.extended_properties
        WHERE class = 1
