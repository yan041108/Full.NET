...
CROSS JOIN (
    VALUES
        (N'identity.users.create'),
        (N'identity.users.update'),
        (N'identity.users.assign_roles'),
        (N'identity.users.reset_password'),
        (N'identity.users.disable'),
        (N'identity.users.enable')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'identity.users.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'identity.users.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'identity.users.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'identity.users.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%identity.users.write%';
