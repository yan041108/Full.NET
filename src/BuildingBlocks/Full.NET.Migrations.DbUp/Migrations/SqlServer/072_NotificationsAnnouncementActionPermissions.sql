...
CROSS JOIN (
    VALUES
        (N'notifications.announcements.create'),
        (N'notifications.announcements.update'),
        (N'notifications.announcements.publish')
) AS actions(PermissionCode)
WHERE legacy.PermissionCode = N'notifications.announcements.write'
  AND NOT EXISTS (
    SELECT 1
...
WHERE PermissionCode = N'notifications.announcements.write';
...
CROSS APPLY (
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE element.value <> N'notifications.announcements.write'
            UNION ALL
            ...
            WHERE writeProbe.value = N'notifications.announcements.write'
        ) AS permissions
    ) AS quoted
) AS rebuilt
WHERE apiKey.PermissionsJson LIKE N'%notifications.announcements.write%';