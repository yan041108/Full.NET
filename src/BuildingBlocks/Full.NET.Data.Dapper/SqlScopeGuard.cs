...
/// SQL 文本必须包含参数占位符 <c>@TenantId</c>（大小写不敏感），防止遗漏 WHERE 条件导致跨租户数据泄露。
...
/// </item>
/// </list>
/// <para><b>为什么校验 @TenantId 文本：</b>
/// 仅靠 Dapper 参数注入无法证明 SQL 真正使用了该参数（可能存在注入后未引用的情况）。
/// 文本级匹配提供了纵深防御，确保 WHERE / ON 子句确实引用租户过滤条件。</para>
