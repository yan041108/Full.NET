    ...
    : ITenantProvisioningService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var localInvalidationOptions = cache.DefaultEntryOptions.Duplicate();
            // 当前请求只负责提交后修复本节点；跨节点通知必须由事务 Outbox 消费者可靠发布。
            localInvalidationOptions.SkipBackplaneNotifications = true;
            // 事务提交后先精确摘除当前节点已知的解析 key，避免本节点在 Outbox 尚未消费前继续命中旧负缓存。
            await cache.RemoveAsync(
                    CacheKeyBuilder.TenantResolutionById(
                        environment.EnvironmentName,
                        tenant.Id),
                    localInvalidationOptions,
