...
internal static class Endpoint
{
    ...
    public static void Map(IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapPost("/{requestId:guid}/retry", async (
            Guid requestId,
            ...
            CancellationToken cancellationToken) =>
        {
            var result = await service.RetryLinkAsync(requestId, request, cancellationToken)
                .ConfigureAwait(false);
            return mapper.Map(result, httpContext);
        })
        .WithName("dataApprovalsRetryRequest")
        .Produces<DataApprovalRequestResponse>(StatusCodes.Status200OK)
        .ProducesProblem(StatusCodes.Status400BadRequest)
        .ProducesProblem(StatusCodes.Status401Unauthorized)
        .ProducesProblem(StatusCodes.Status403Forbidden)
        .ProducesProblem(StatusCodes.Status404NotFound)
        .ProducesProblem(StatusCodes.Status409Conflict)
        .RequireAuthorization(FullNetPermissionPolicies.For(DataApprovalPermissions.Retry));

        group.MapPost("/{requestId:guid}/retry-apply", async (
            Guid requestId,
            RetryDataApprovalRequestBody request,
            DataApprovalRequestService service,
            IApiResultMapper mapper,
            ...
            CancellationToken cancellationToken) =>
        {
            var result = await service.RetryApplyAsync(requestId, request, cancellationToken)
                .ConfigureAwait(false);
            return mapper.Map(result, httpContext);
        })
        .WithName("dataApprovalsRetryApplyRequest")
        .Produces<DataApprovalRequestResponse>(StatusCodes.Status200OK)
        .ProducesProblem(StatusCodes.Status400BadRequest)
        .ProducesProblem(StatusCodes.Status401Unauthorized)
        .ProducesProblem(StatusCodes.Status403Forbidden)
        .ProducesProblem(StatusCodes.Status404NotFound)
        .ProducesProblem(StatusCodes.Status409Conflict)
        .RequireAuthorization(FullNetPermissionPolicies.For(DataApprovalPermissions.RetryApply));
    }
