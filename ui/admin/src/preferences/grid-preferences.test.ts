...
describe('Vue Grid 偏好适配器', () => {
  const columns = [
    { key: 'phone', width: 140, hidden: false, fixed: false },
    { key: 'gender', width: 88, hidden: false, fixed: false },
    { key: 'createdAt', width: 180, hidden: false, fixed: false }
  ];
  ...
  it('恢复顺序、宽度、可见性和冻结状态且不修改源列', () => {
    const preference: GridPreferenceResponse = {
      ...
      columns: [
        ...
        { columnKey: 'phone', order: 1, width: 260, visible: true, fixed: 'left' }
      ],
      version: 4
    };
    ...
    expect(restored[0]).toMatchObject({ width: 200, hidden: true, fixed: 'right' });
    expect(restored[1]).toMatchObject({ width: 260, hidden: false, fixed: 'left' });
  ...
  it('SchemaVersion 变化时 load 与 apply 都安全回退', async () => {
    const restored = applyGridPreference('identity.users', columns, {
      ...
      schemaVersion: 1,
      columns: [
        { columnKey: 'phone', order: 0, width: 999, visible: false, fixed: 'left' }
      ],
      version: 9
    });
