...
<template>
  <section class="dict-types-view art-page-stack art-full-height" :aria-busy="loading">
    ...
    <el-card
      v-if="selectedType"
      class="art-table-card dict-types-items-card"
      shadow="never"
      data-dict-items-panel
      :aria-busy="itemsLoading"
    >
      <div class="dict-types-items-card__header">
...
<style scoped>
.dict-types-view :deep(.art-table-card) {
  flex: 1;
  display: flex;
...
.dict-types-view :deep(.art-table-card .el-card__body) {
  display: flex;
  flex: 1;
  flex-direction: column;
  min-height: 0;
...
.dict-types-items-card {
  margin-top: 12px;
}

.dict-types-items-card__header {
  display: flex;
  align-items: center;
...
.dict-types-items-card__header h2 {
  margin: 0;
  font-size: 16px;
  font-weight: 600;
}
