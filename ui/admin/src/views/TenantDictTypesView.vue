...
defineOptions({ name: 'TenantDictTypesView' });
...
async function fetchAllDictTypes(): Promise<SettingsDictType[]> {
  const pageLimit = 100;
  const firstPage = await listSettingsTenantDictTypes(1, pageLimit);
  const items = [...firstPage.items];
  const totalPages = Math.ceil(firstPage.total / pageLimit);
  for (let current = 2; current <= totalPages; current += 1) {
    const nextPage = await listSettingsTenantDictTypes(current, pageLimit);
...
async function fetchAllDictItems(dictTypeId: string): Promise<SettingsDictItem[]> {
  const pageLimit = 100;
  const firstPage = await listSettingsTenantDictItems(dictTypeId, 1, pageLimit);
  const items = [...firstPage.items];
  const totalPages = Math.ceil(firstPage.total / pageLimit);
  for (let current = 2; current <= totalPages; current += 1) {
    const nextPage = await listSettingsTenantDictItems(dictTypeId, current, pageLimit);
    items.push(...nextPage.items);
  }
  return items;
}
...
<template>
  <section class="tenant-dict-types-view art-page-stack art-full-height" :aria-busy="loading">
    ...
    <ArtFormDialog
      ...
      confirm-test-id="tenant-dict-types-editor-submit"
      :show-confirm="typeEditorMode === 'create' ? canCreate : canUpdate"
      @confirm="submitTypeEditor"
    >
      <el-form
        ref="typeEditorFormRef"
        data-testid="tenant-dict-types-editor-form"
        :model="typeEditorForm"
        label-width="96px"
        class="tenant-dict-types-editor-form"
      >
        <el-form-item
