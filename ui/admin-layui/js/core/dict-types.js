...
export function createDictTypesController(root, options) {
  ...
  const form = root.querySelector('[data-dict-types-create-form]');
  const directory = root.querySelector('[data-dict-types-directory]');
  const itemsPanel = root.querySelector('[data-dict-items-panel]');
  const itemsForm = root.querySelector('[data-dict-items-create-form]');
  const itemsDirectory = root.querySelector('[data-dict-items-directory]');
  const itemsPanelTitle = root.querySelector('[data-dict-items-panel-title]');
  const itemsClose = root.querySelector('[data-dict-items-close]');
  let loading;
  let changing = false;
  let selectedTypeId;
  ...
  const load = async () => {
    ...
    loading = request('/api/v1/settings/dict-types?page=1&pageSize=20')
      .then(page => {
        renderDirectory(
          directory,
  ...
  const loadItems = async () => {
    ...
    try {
      const page = await request(
        `/api/v1/settings/dict-types/${encodeURIComponent(selectedTypeId)}/items?page=1&pageSize=20`
      );
  ...
  const onCreateItem = async event => {
    ...
    try {
      await request(
        `/api/v1/settings/dict-types/${encodeURIComponent(selectedTypeId)}/items`,
        jsonRequest({
          label,
          value,
          color: color || null,
  ...
  const onDirectoryAction = event => {
    const manageButton = event.target instanceof Element
      ? event.target.closest('[data-dict-types-items]')
      : undefined;
    if (manageButton && !changing) {
      void openItems(
        manageButton.dataset.dictTypesItems,
    ...
    const editButton = event.target instanceof Element
      ? event.target.closest('[data-dict-types-edit]')
      : undefined;
    if (editButton && !changing) {
      const dictTypeId = editButton.dataset.dictTypesEdit;
      ...
      promptText(translation().t('dictTypes.editTitle'), currentName, async nextName => {
        ...
        try {
          await request(
            `/api/v1/settings/dict-types/${encodeURIComponent(dictTypeId)}`,
            {
              method: 'PUT',
    ...
    const disableButton = event.target instanceof Element
      ? event.target.closest('[data-dict-types-disable]')
      : undefined;
    if (!disableButton || changing) return;
    const dictTypeId = disableButton.dataset.dictTypesDisable;
    ...
    confirmAction(message, async () => {
      ...
      try {
        await request(
          `/api/v1/settings/dict-types/${encodeURIComponent(dictTypeId)}/disable`,
          { method: 'POST' }
        );
  ...
  const onItemsDirectoryAction = event => {
    const editButton = event.target instanceof Element
      ? event.target.closest('[data-dict-items-edit]')
      : undefined;
    if (editButton && !changing) {
      const dictItemId = editButton.dataset.dictItemsEdit;
      const version = Number(editButton.dataset.version ?? '0');
      const displayOrder = Number(editButton.dataset.displayOrder ?? '0');
      ...
      promptText(translation().t('dictItems.editTitle'), currentLabel, async nextLabel => {
        ...
        try {
          await request(
            `/api/v1/settings/dict-items/${encodeURIComponent(dictItemId)}`,
            {
              method: 'PUT',
              headers: { 'content-type': 'application/json' },
              body: JSON.stringify({
    ...
    const disableButton = event.target instanceof Element
      ? event.target.closest('[data-dict-items-disable]')
      : undefined;
    if (!disableButton || changing) return;
    const dictItemId = disableButton.dataset.dictItemsDisable;
    ...
    confirmAction(message, async () => {
      ...
      try {
        await request(
          `/api/v1/settings/dict-items/${encodeURIComponent(dictItemId)}/disable`,
          { method: 'POST' }
        );
...
function renderDirectory(container, dictTypes, translation) {
  ...
  dictTypes.forEach(dictType => {
    ...
    manage.dataset.dictTypesItems = dictType.id;
...
function showProblem(root, problem, fallback) {
  const panel = root.querySelector('[data-dict-types-problem]');
  if (!panel) return;
  panel.hidden = false;
