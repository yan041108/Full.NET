...
- 实现状态：现有 Bootstrap 账号只同步“执行引导时已注册的 Host 权限”；本文定义的动态全权限语义、保护门禁和双端管理尚未实现
...
现有 Bootstrap 近似采用该方案：把当时全部 Host 权限写入 `fn_identity_role_permission`。优点是沿用普通 RBAC；缺点是新模块权限会漂移，只有再次同步后才完整，租户作用域权限也无法自然表达。它不能单独满足“默认永远拥有全部权限”。
