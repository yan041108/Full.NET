...
RunId 服从 [ADR-0003](../../architecture/adr/ADR-0003-uuid-v7-primary-key-storage.md) 的统一主键存储：应用在运行开始前生成 UUID v7，Contributor、Seeder 和业务模块只处理 C# `Guid`，不得自行进行 MySQL 字节转换。已实现的 007 Seed 表中，SQL Server 使用 `uniqueidentifier`，MySQL 暂时使用 `char(36)`；后者已纳入 008/009 UUID Binary16 存量转换，完成后目标类型为 RFC 字节序 `BINARY(16)`。
