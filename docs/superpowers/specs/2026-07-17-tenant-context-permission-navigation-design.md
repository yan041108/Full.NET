...
1. 使用最小 RBAC 表表达普通授权；后续批准的超级管理员由受保护系统角色标记并从授权目录动态投影权限，不使用用户名、通配符或无条件授权；
...
本节记录了已实现切片和升级方向。超级管理员的最后一名保护、Claim、作用域和双端验收以[新设计](2026-07-18-super-administrator-design.md)及[实施计划](../plans/2026-07-18-super-administrator.md)为准；在该计划完成前，当前能力仍只是显式权限同步的宿主管理员。
...
切换不改变宿主管理员的演员身份；普通管理员不增加角色权限，超级管理员升级完成后则按切换后的可信作用域重新投影适用权限。两者都只在选中租户内查询数据。因此切换前尚未过期的 Host Token 不会形成新的跨租户数据路径。后续租户委派权限必须建立独立设计。
