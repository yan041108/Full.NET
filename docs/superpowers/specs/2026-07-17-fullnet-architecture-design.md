...
### 6.12 跨调用恢复与资源清理边界（2026-09-08 复审修复）

- K3Cloud 保存与提交分开调用，保存返回的单据标识/编号必须先以本模块短事务持久化，再提交同一单据。确定的 Submit 失败只重试 Submit；未知结果和过期 pending 不代表远端未执行，必须先由运维对账，不允许重新 Save。当前没有自动远端对账入口，不得宣称未知状态已自动恢复。
- Organization 以可信任务标识、原始行号及载荷摘要维护岗位导入回执。唯一键占位、岗位创建、机构/职级关联与回执完成属于同一本地事务；失败回滚整行，成功重放返回原岗位标识。ImportExport 只传递任务标识和调用公开处理器，不参与 Organization 事务，也不读取回执表。
- 209 双库前向迁移仅新增 Organization 回执表。升级前须停止并排空旧导入 Worker；旧版本已部分写入但未提交检查点的任务没有回执证据，必须对账后再恢复，不得把同编码岗位直接当作本任务的成功结果。
- 租户资源文件先标记 released 撤销读取，再删除物理对象；released 必须持续参与对账，物理删除成功后才移除墓碑。删除失败保留墓碑，不阻塞后续条目。
- 文件对账使用同一 Worker 内共享、互斥的跨作用域扫描游标，受单轮批次预算限制时下轮继续，到尾部后回绕；重启从头扫描。游标只用于公平调度，文件归属与租户授权仍由数据库和资源所有者判定。
...
按功能矩阵持续交付官方扩展、Provider、Sample 和 Client，包括 Workflow 与 Notifications 平台扩展的剩余切片，以及 Flutter 原生移动/桌面客户端、按需 MAUI 模板、在线构建、导入导出、报表、微信、支付、OAuth、APIJSON、数据库视图、ES 日志、MQTT、AI、Agent、MCP、Agentic Web、审批、钉钉、GoView、K3Cloud、OCR、ReZero 和企业微信等。各项具体实现与验证状态以当前专项记录为准，不把本清单视为全部尚未开发。每个子模块独立完成计划、实现和验收，不阻塞核心 1.0 发布。

每个里程碑都必须保持可构建、可测试、可演示，不允许长期维护一个无法运行的大分支。

### 24.1 企业应用与 SaaS 底座完善（2026-09-16）

项目所有者已要求将八项补齐能力及已有模块收口重点纳入规划，执行入口为[底座完善开发计划](../plans/2026-09-16-foundation-productization.md)，能力归属见[路线图 B01—B08](../../roadmap/adminnet-feature-parity.md#8-企业应用与-saas-底座完善队列2026-09-16)。本次确认建设方向与交付边界，不把规划批准作为代码或生产验收证据。

1. M3 优先补齐项目创建/诊断、企业开通与成员生命周期、账号自助恢复、套餐权益/统一配额及完整业务样板；M4 收口所选预设的升级恢复与运行验收；订阅运营和开放集成为可选官方能力，按对应预设推进，不将八项全部追加为核心 1.0 必选项。
2. Identity 是账号、认证和企业成员授权的权威源；Organization 管理组织隶属；Tenancy 管理企业生命周期、套餐、权益、配额及订阅编排；Payments 管理支付/退款事实。扩展原有关系和 Port，不复制用户或创建竞争的成员事实源。
3. 权益、用户权限与使用配额分别判断。业务套餐权益不进入通用 Settings ConfigEntry；硬配额使用可追踪预留/确认/释放与对账，不靠跨模块本地事务或陈旧缓存保证正确性。
4. 企业开通、停用、支付履约和配额消费使用有状态、幂等、可恢复的模块协作；外部调用不在本地数据库事务中，未知执行结果不能误报成功或自动释放额度。
5. 基础管理、企业应用和 SaaS 是已有模块组合的交付预设，不是三个独立框架。默认关闭未验收的外部渠道，只有实际消费者才新增可选模块或拆分 Contracts。企业所有者不是 Host 超级管理员。
6. 已有 SSO、AI、安全修复、Workflow、通知等专项继续持有实现步骤；总计划只管理依赖与阶段出口。新增功能同时补齐 API、Vue、权限/租户、异步行为、双库和适用 Native 证据。
7. 保持 Kubernetes/Helm 生产参考、MIT、Native AOT、Dapper 与双库基线；公共注册默认受政策控制，真实收费、通知发送和不可逆数据清理不由文档更新自动执行。
8. 首版独立应用通过固定提交、版本和摘要的源码分发包创建：框架源码及工具/迁移/前端共享依赖在应用内受管目录保存，应用宿主和业务代码单独拥有；不依赖原始开发仓库或尚未发布的自有包。按所选预设投影静态 Composition，升级以 manifest 比对并保留人工修改。
9. 默认邀请制包含未注册者入驻：受控身份挑战投递不要求已有账号或租户成员，普通通知目录与验证边界不放宽；邮箱验证后创建/绑定账号，再激活成员，既有账号仍需原认证/MFA。注册原语在 F04 完成，正式邀请入口在 F05 接通。
10. 权益在存量部署使用 Compatibility → Shadow → Enforced 的受控迁移，新 SaaS 预设直接要求 Enforced 与有效绑定；存量功能清单和权益回填不得扩大原有权限。强制阶段状态故障不回退为兼容，切换前验证所有相关实例；升级和恢复不能绕过已执行的付费限制。
11. 成员基础能力 F05 不依赖配额任务；F08a 先交付配额协议，F08b 再接真实成员/文件入口及存量用量。未启用配额的基础预设可先验收，限额/SaaS 预设须等 F08b，不能以错误时降级无限额替代接入。
