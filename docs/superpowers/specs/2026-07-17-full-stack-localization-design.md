...
1. 当前会话中用户刚刚明确选择且尚在同步的语言；
2. 已认证账号的 PreferredLocale；
3. 匿名阶段本地保存的明确选择；
4. 浏览器、uni-app 或 Flutter 设备语言；
5. Full.NET 默认 zh-CN。

用户在客户端切换语言时：

- 立即在本地更新界面和后续请求的 Accept-Language；
- 匿名状态只保存本地偏好；
- 已认证状态异步更新账号 PreferredLocale；失败时保持本次会话语言并显示可重试提示，不能退出登录；
