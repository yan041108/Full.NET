...
> **For agentic workers:** Use `fullnet-module-delivery`. Execute task-by-task with RED first. Steps use `- [ ]` checkboxes.
...
- [ ] **Step 1:** RED — 权限常量、`rollback` operationKind、错误码与 051 恢复测试因缺失而失败。
- [ ] **Step 2:** 实现契约、授权贡献、双库 051（含成功 Rollback 唯一约束与 `ArtifactCount >= 0` 成功语义）。
- [ ] **Step 3:** naming + 051 双库恢复 GREEN。
...
- [ ] **Step 1:** RED — 资格拒绝、busy、checkpoint 缺失、成功恢复摘要。
- [ ] **Step 2:** 实现服务：共享 Gate → insert running → Read/Restore → complete/fail。
- [ ] **Step 3:** Unit GREEN；冲突计划零写入。
...
- [ ] **Step 1:** RED — 403/资格/成功 Rollback 双 Provider。
- [ ] **Step 2:** 接线 Endpoint 与 OpenAPI。
- [ ] **Step 3:** affected inner CodeGeneration GREEN。
...
- [ ] **Step 1:** RED — 契约与双端权限可见性。
- [ ] **Step 2:** 确认对话框与错误码处理。
- [ ] **Step 3:** 真实栈 Apply→Rollback E2E GREEN（SQL Server；MySQL 按现有矩阵）。
...
- [ ] **Step 1:** affected slice + naming + `git diff --check`。
- [ ] **Step 2:** 记录证据；明确未交付保留清理/多实例。
- [ ] **Step 3:** 规则/Skill 演进一行结论；本计划勾选。

## Acceptance Checklist

- [ ] DB succeeded Apply 是唯一资格权威。
- [ ] 共享 Gate；复用 RollbackWorkspace；不删检查点。
- [ ] 051 双库恢复；独立 `codegen.runs.rollback`。
- [ ] Vue + Layui + 真实栈 E2E。
- [ ] 文档不把内部执行器宣传为完整 Verified Rollback。