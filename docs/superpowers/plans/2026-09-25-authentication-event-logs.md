...
- 已补充 TOTP 绑定开始、确认成功与拒绝，密码加 TOTP 的强制再认证结果，密码恢复成功与挑战重放拒绝，以及 Host 管理员重置密码成功事件。敏感验证码、密码和恢复码不入审计。绑定/恢复/管理重置的成功状态与事件在 Identity 本地事务中提交；拒绝事件独立保留；审计写入异常按失败处理。
- 查询改为最近 24 小时的固定时间窗及 `(OccurredAtUtc DESC, Id DESC)` 游标翻页；游标绑定筛选条件，单页最多 100 条。Vue 页面支持前后页与页大小，OpenAPI 快照和客户端已再生成。
- SQL Server/MySQL 集成验证已覆盖同时间戳游标边界、无效/跨筛选游标、TOTP 拒绝及成功、密码恢复重放、管理员重置和两个 Worker 并发保留批次。实际命令与结果记入 [验证记录](../../verification/2026-09-25-authentication-events-phase2.md)。
- 真实浏览器页面/详情与延迟旧查询的竞态回归已通过（2/2）；Linux Native AOT 分析器与发布已通过，但原生 API 运行烟测在本地 Docker 网络报连接错误，仍为未通过。双库 SSO 会话复用的审计关联断言、保留任务模拟数据库超时后的重试已通过。还需覆盖更多协议拒绝、进程/网络级故障注入与完整事件目录，不能把这轮局部通过视为 AE01—AE06 全部完成。
