...
- 2026-09-13（续 3）：`pnpm openapi:client:snapshot` 与 `--check` 通过（SqlServer/MySQL 运行时快照）；`pnpm test:openapi` 中相对 `HEAD` 的 breaking gate 因未提交契约仍失败（提交后应归零）。MCP 集成专测修复：`McpLoopbackHttpClientTestSupport` 将 `McpClientConnectionManager` 出站 HTTP 绑定到 `WebApplicationFactory` 测试宿主；`McpServerAuthorization` 补 `application/json` Content-Type；`McpServerInterop` 资源摘要改为 JSON 字段断言。本地顺序验证通过：`McpServerAuthorization`、`McpServerInterop`、`McpClientInterop`、`McpRemoteConnectionApi`（SqlServer；MySQL 受 Docker 备份争用影响需单独重跑）。
- 未执行/未关闭：Linux publish/原生进程、完整迁移恢复演练、旧 Provider 退役、Vue 页面集中验收、全量 `pnpm test:integration:partitions`。T15 整体未关闭。
