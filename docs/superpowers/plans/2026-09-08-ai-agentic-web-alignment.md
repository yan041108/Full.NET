...
- 2026-09-14（续 19）：根因定位为 Messaging 订阅目录 Scoped DI 环：`IntegrationEventSubscriptionCatalog` 构造解析 `OrganizationUnitChangedKafkaSubscription` 时枚举全部 `IIntegrationEventHandler`，经 `NotificationIntentService`→`IOutboxWriter`→`EffectiveEventDeliveryOwnerResolver`→目录再次解析而挂起。修复：`EffectiveEventDeliveryOwnerResolver` 延迟访问目录；Kafka 订阅改为直接注入 `OrganizationUnitChangedIntegrationEventHandler`；Native E2E replay 断言附带原生日志。待推 CI 验证 `critical_http_flow` 与 `workflow_approval_flow`。
