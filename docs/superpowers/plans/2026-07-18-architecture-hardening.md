...
**状态：Task 8A、Task 8B1 与 Sink 单事件故障隔离已完成。** 普通日志与 `Error/Critical` 已使用独立有界异步队列，高优先级通道固定非阻塞并提供独立队列深度、容量、丢弃指标与 ready 降级检查；两条 Worker 在退出时并行排空并共享一个 5 秒默认总预算，高优先级先获得等待权，超时不会把宿主退出拖成每通道各等待一次。内部 Sink 的真实异常会传播到 Worker；失败事件计入对应通道 dropped 后，Worker 继续消费后续事件，SelfLog 只记录异常类型。共享队列过载、高优先级慢 Sink、正常排空、双 Sink 同时阻塞及普通/高优先级单事件失败恢复已由聚焦 Unit 锁定。审计仍由业务数据库事务或 Outbox 持久化，不进入日志队列。磁盘 Spool、外部可靠 Sink、连续平台不可用、磁盘满、跨重启重放与投递确认仍属于后续 Task 8B，因此完整 Task 8 尚未完成。

**Task 8A 详细计划：** [`2026-07-26-high-priority-logging-channel.md`](2026-07-26-high-priority-logging-channel.md)

**Task 8B1 详细计划：** [`2026-07-26-bounded-logging-shutdown.md`](2026-07-26-bounded-logging-shutdown.md)
