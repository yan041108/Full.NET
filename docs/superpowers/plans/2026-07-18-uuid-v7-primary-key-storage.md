...
**Architecture:** 应用继续通过 `IIdGenerator` 在写库前生成 UUID v7。新增 `Full.NET.Data.MySql` Provider 边界统一构造 MySqlConnector 连接串并强制最终模式 `GuidFormat=Binary16`；业务模块只处理 `Guid`。MySQL 使用 007 Expand 添加/回填二进制影子列并以触发器保持旧应用写入同步，维护窗口内执行 008 Contract 原子化切换列名、约束和连接模式。SQL Server 不改变 `uniqueidentifier`，但在 008 中显式治理主键与高写入表聚集索引。001-006 历史迁移保持不变。
...
- 本计划拥有迁移编号 `007_UuidBinaryExpand` 与 `008_UuidBinaryContract`；命名规范化顺延为 009/010，Seeding 顺延为 011。
- 007 和 008 必须在 Naming Expand、Seeding 表以及任何新的 UUID 持久化列之前完成；执行期间禁止其他分支新增迁移。
...
- 007 Expand 不删除或改名旧列；008 Contract 只有在停止写入、备份、数据核对、回退演练和批准的破坏性 DDL 豁免全部成立后才能执行。
- 008 后回退旧应用需要恢复数据库备份，不能只回滚二进制；Runbook 必须把这一事实写入 Go/No-Go 门禁。
- 完成前必须验证空库 001→008、带真实关系数据的 001-006→008、007 半完成重跑、冲突数据拒绝、双库应用 API/Worker/Outbox 路径。
...
**Interfaces:**
- Consumes: 001-006 SQL Server/MySQL 迁移、ADR-0003、当前 `DatabaseOptions`
...
`FamilyId`、`SessionId`、`TenantId` 等即使没有数据库外键也属于 UUID 引用，不能只扫描约束。清单还必须声明 009/010 命名变化，避免后续把旧表名当成新模板。
...
Runbook 按顺序规定：冻结迁移；备份并验证可恢复；停止 API/Worker；等待事务与 Outbox lease 到期；记录行数/外键/摘要；运行 007 或确认已运行；执行 delta backfill；执行 008；部署 Binary16 应用；运行双库冒烟；开启流量。任一步失败均停止；008 前可删除影子对象并继续旧应用，008 后必须恢复备份和旧应用。
...
单元测试要求：`LegacyChar36` 只用于 001-007 过渡；`Binary16` 总是输出 `GuidFormat=Binary16`；输入含 `Char36`、`TimeSwapBinary16` 或冲突 GuidFormat 时拒绝而不是静默覆盖；迁移连接只额外启用 `AllowUserVariables=true`；错误不回显连接串或 Secret。
...
### Task 3: 实现 007 MySQL UUID Binary Expand

**Files:**
- Create: `src/BuildingBlocks/Full.NET.Migrations.DbUp/Migrations/SqlServer/007_UuidBinaryExpand.sql`
- Create: `src/BuildingBlocks/Full.NET.Migrations.DbUp/Migrations/MySql/007_UuidBinaryExpand.sql`
...
**Interfaces:**
- Consumes: Task 1 UUID 图、001-006 数据库、Task 2 `LegacyChar36`
- Produces: 每个 MySQL UUID 列的 `BINARY(16)` 影子列、完整回填和 legacy-write 同步触发器；旧列保持可用

- [ ] **Step 1: 先写存量数据与半完成恢复 RED 测试**

测试写入多租户用户、角色关系、刷新令牌 family/replacement、审计 Actor/Session、Pending/Locked Outbox，再执行 007。分别模拟影子列仅创建一部分、回填一部分、触发器缺失和 DbUp 未记账重跑。SQL Server 配对脚本必须安全通过且不伪造类型转换。
...
为仍以 `char(36)` 写入的表建立命名稳定的 `BEFORE INSERT/UPDATE` 触发器，只执行文本到 Binary16 的 RFC 转换。触发器发现源/影子同时存在但不一致时必须 `SIGNAL` 失败，不得覆盖冲突。007 后旧应用继续运行时，新写入和更新必须保持影子列一致。
...
### Task 4: 冻结写入并执行 008 Contract 切换

**Files:**
- Create: `src/BuildingBlocks/Full.NET.Migrations.DbUp/Migrations/SqlServer/008_UuidBinaryContract.sql`
- Create: `src/BuildingBlocks/Full.NET.Migrations.DbUp/Migrations/MySql/008_UuidBinaryContract.sql`
...
**Interfaces:**
- Consumes: 已核对的 007、停止 API/Worker 写入的维护窗口、已验证备份
- Produces: MySQL canonical UUID 列全部为 `BINARY(16)`；Host 全部使用 `Binary16` 模式；legacy 触发器/文本列退出

- [ ] **Step 1: 写 Contract 前置条件 RED 测试**

008 必须拒绝：007 未完成、任一影子 NULL/冲突、引用不一致、同步触发器缺失、数据库仍标记为允许 legacy writer、维护窗口令牌缺失或备份确认未登记。测试覆盖 008 在删除部分约束、重命名部分列后未记账的恢复路径。
...
SQL Server 保持 `uniqueidentifier`。008 为现有主键补齐稳定名称和显式 `CLUSTERED/NONCLUSTERED`：Outbox、Auth Audit 使用 UUID 非聚集主键，并按当前时间列＋`Id` 建立显式聚集索引；关系表按主导连接顺序显式聚集；低写入实体保留/重建显式 UUID 聚集主键。变更前后执行计划、页分裂和碎片基准保存在验证记录，未达到门禁时 008 不得发布。

- [ ] **Step 5: 切换所有 Host 配置**

API、Worker、Migrator、AppHost 和集成测试默认目标模式改为 `Binary16`。`LegacyChar36` 只允许受控恢复工具显式使用，Production 启动必须拒绝 legacy。对数据库 Schema 版本做启动检查：应用处于 Binary16 而 008 未完成，或数据库已 008 而应用处于 legacy，均在接收流量前失败。
...
Expected: 空库、001-006 升级、007 在线增量、五类 008 半完成、冲突拒绝和 SQL Server 聚集策略全部 PASS；测试必须确认旧应用配置在 008 数据库上启动失败。
...
**Interfaces:**
- Consumes: 008 数据库与 Binary16 连接策略
...
扫描新增/修改 MySQL 迁移，禁止 UUID 列使用 `char(36)`；扫描业务代码禁止手工 UUID 字节转换；扫描 SQL Server 新主键必须显式 `CLUSTERED`/`NONCLUSTERED`，并要求高写入表登记聚集索引用途。历史 001-006 只允许精确路径豁免，008 完成后删除该豁免。
...
- Produces: 可复制配置、真实能力状态、后续 009/010/011 迁移解锁
...
使用含真实关系数据与 Pending Outbox 的备份演练两条路径：007 前/后回退旧应用；008 后恢复数据库备份并回退旧应用。记录耗时、数据摘要、RPO/RTO 和无法仅靠应用回滚的事实。
...
只有 Tasks 1-3 完成时标记 `Implemented`；008、全路径双库和恢复验证完成后可标记 `Build-verified`；只有生产等价维护窗口与 SQL Server 索引证据全部完成才标记 `Verified`。更新 Getting Started，说明 MySQL 必须使用 Binary16 模式和人工排障时使用 `BIN_TO_UUID(..., 0)`。

- [ ] **Step 5: 解锁后续迁移**

确认 007/008 已合并且任何未发布分支没有占号后，才执行 009/010 命名规范化和 011 Seeding。若实际迁移编号改变，三个计划、合同和所有测试必须在同一提交同步，禁止仅改文件名。
...
6. 001-006 升级、007/008 半完成恢复、冲突拒绝和备份恢复演练有可定位证据；
7. 009/010/011 后续计划编号无冲突，能力矩阵没有把未验证目标写成现状；
