...
- Scope A 独立 Confluent.Kafka Runner、Scope B `worker_inbox_handler` 与 Scope C `transaction_outbox_cdc` 已实现；Scope C 测量 Outbox→Debezium→Kafka→Inbox 全链路并复用 Scope B 消费内核。MySQL + Connect 缩减集成测试已通过；正式容量矩阵与 Soak 尚未运行，状态仍为 `Capacity-not-verified`。
