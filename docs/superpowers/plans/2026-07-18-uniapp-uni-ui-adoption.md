...
- [x] **Step 1: 写主题契约失败测试**
- [x] **Step 2: 在 `uni.scss` 定义 `$uni-color-primary` 等标准变量，并在适配样式中使用 Full.NET 语义命名**
- [x] **Step 3: 禁止业务页面直接覆盖 `.uni-*` 内部结构选择器**
- [x] **Step 4: 运行单测和类型检查**
...
- [x] **Step 1: 写组件清单和本地化文本失败测试**
- [x] **Step 2: 实现只在 Development/Test 可导航的冒烟页**
- [x] **Step 3: H5 E2E 验证中文/英文、键盘焦点、错误提示和 320 CSS px 布局**
- [x] **Step 4: 分别构建三个目标**
...
- [x] **Step 1: 增加保存失败、并发和禁用状态测试**
- [x] **Step 2: 用 uni-ui 表单/列表/反馈组件替换视觉层，不改变 Model/API**
- [x] **Step 3: 运行 96 项现有测试、新增测试和 H5 E2E**
...
- [x] **Step 1: 执行依赖漏洞、许可证和包体积检查**
- [ ] **Step 2: 在微信/支付宝开发者工具分别验证组件样式、弹层、输入和语言切换**（当前机器未安装两个开发者工具）
- [x] **Step 3: 没有真机证据时保持 `Build-verified`，不得标为 `Verified`**
- [x] **Step 4: 更新路线图和验证记录**
