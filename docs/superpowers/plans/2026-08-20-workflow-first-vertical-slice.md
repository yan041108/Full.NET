...
- [x] SQL Server/MySQL 各覆盖 Host 和 Tenant：approve、reject、无权限按钮缺失、直接 API 403、旧 Revision 409、危险 Patch 422 和刷新后权威状态。Tenant Vue「按钮不进 DOM」由 Host 同一 `WorkflowTodosView` 覆盖，Tenant 403 以隔离 API 矩阵验收；见 [`2026-08-31-workflow-first-slice-closeout.md`](../../verification/2026-08-31-workflow-first-slice-closeout.md)。
...
- [x] 使用任务快照先运行受影响 slice：收口窗口为 `pnpm test:slice -- --snapshot workflow-first-slice-closeout-20260831`；禁止本地全量替代受影响集。见 [`2026-08-31-workflow-first-slice-closeout.md`](../../verification/2026-08-31-workflow-first-slice-closeout.md)。
- [x] 新建 dated Verification，记录基线、环境、命令、非零发现数、原始结果、容量未验证项和许可证状态。Host Vue 管理端见 [`2026-08-30-workflow-admin-approval-real-stack.md`](../../verification/2026-08-30-workflow-admin-approval-real-stack.md)；Host/Tenant × 双库矩阵收口见 [`2026-08-31-workflow-first-slice-closeout.md`](../../verification/2026-08-31-workflow-first-slice-closeout.md)。
- [x] 依据新鲜实现和验证证据将 Workflow 更新为最高真实状态 `Build-verified`；Tenant/422 已关闭。Worker、网关/抄送和容量门禁开放期间不得提升为 `Verified`。
