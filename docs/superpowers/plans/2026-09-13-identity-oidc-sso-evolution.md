...
现有 Identity 已有登录/退出等认证审计写入；统一事件覆盖、查询及 Vue 页面由[认证事件日志 AE01—AE06](2026-09-25-authentication-event-logs.md)集中跟踪。SSO 任务需提供中心/应用会话、免密复用、应用退出与中心退出的准确事件来源，不再建立平行日志表。该专项已有首条纵向切片，完整事件矩阵仍未验收，不据此提升本计划的验收状态。
