...
实施时数据库真实基线只有 001-004，而命名规范化尚未开始。超级管理员角色使用 `005_SuperAdministrator.sql`，可追责审计操作者列与索引使用 `006_SuperAdministratorAuditActor.sql`；未实施的命名 Expand/Contract 顺延为 007/008。禁止修改已经执行或发布的迁移。
