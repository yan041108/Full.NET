...
dotnet tests/Full.NET.UnitTests/bin/Release/net10.0/Full.NET.UnitTests.dll --minimum-expected-tests 333
...
| 发布 | 合入 `main`、发布候选 | 全量 `--minimum-expected-tests 107 --timeout 90m` |
...
# 发布：完整双库矩阵
dotnet tests/Full.NET.IntegrationTests/bin/Release/net10.0/Full.NET.IntegrationTests.dll --minimum-expected-tests 107 --timeout 90m
```
