...
同日刷新参考仓库至 `09d38bd8` 后又审计 31 个提交：新增明确缺口为 Identity 用户 Excel 模板/文件导入体验和 MCP 安全/AOT 设计；Workflow 已按批准的自有内核与不可变版本边界交付多级审批、可执行 `notify.cc`、Tenant 本地收件人候选、首个可执行 `gateway.exclusive`、活动待办安全改派、实例暂停/普通恢复/管理员强制恢复、Assigned/Completed/Rejected/Cancelled 异步通知投影、四个内建站内信模板的可信作用域原子初始化、双端运行时、原生表单设计器、Host/Tenant 权限并发表单安全矩阵及双库 Native AOT 静态证据，状态为 `Build-verified`。Worker 端到端投影和生产容量仍开放；Recovery Worker 控制面已交付但 Linux 原生进程、双库套件和页面真实栈尚未关闭。日志流式读取进一步确认 Observability Admin 缺口，但参考实现的路径拼接与本机 HTTP 回环均不得直接吸收。
...
| 邮件与短信 | Notifications Providers | Provider | M5+ | Implementing（SMTP 邮件 Provider 已 **Build-verified**：MailKit 4.17.0、465/SSL 与 587/STARTTLS 模式、环境变量 Secret、条件注册和已验证收件端点；当前用户邮箱端点登记/查询/删除 API 与 Vue 管理页已 **Build-verified**，但新端点固定为 `pending` 且不会参与投递。QQ SMTP 实测连接成功但认证被拒，标记 **External-auth-not-verified**，不声明邮件已发送。邮件验证码、自动升级 `verified`、生产账号认证、容量与送达回执仍开放；短信仍为 Mapped。见[SMTP 验证](../verification/2026-09-01-notifications-smtp-provider.md)和[收件端点验证](../verification/2026-09-02-notifications-recipient-endpoint-management.md)） |
...
| 消息中心 | Notifications | Core | M3 | **Build-verified**（现有 Host 发信 + 个人收件箱/未读/已读）；平台内核、Tenant Inbox、模板/Intent、多 Profile/Binding、Delivery Worker、Workflow 可靠提醒事件消费与四个内建站内信模板、Vue 管理控制面均保持 **Build-verified**，见[平台收口验证](../verification/2026-08-31-notifications-platform-closeout.md)和[Workflow 异步提醒投影](../verification/2026-09-05-workflow-notifications-event-projection.md)。首个 `email.smtp` Provider 已完成代码、双库与 AOT 分析验证，见[SMTP 验证](../verification/2026-09-01-notifications-smtp-provider.md)；当前用户邮箱端点登记/查询/删除 API 与 Vue 管理页已完成受信作用域、掩码和精确权限验证，见[收件端点验证](../verification/2026-09-02-notifications-recipient-endpoint-management.md)。QQ 外部账号仍为 **External-auth-not-verified**，Worker 端到端投影、端点验证闭环、生产容量和送达回执未关闭，不复制 `object Extras`。 |
...
| Kafka / CDC Relay / EventBus 事件集成 | EventDelivery Provider | Provider | 提前实施计划 | **Build-verified / Pilot**（Organization 真实 CDC E2E + 故障矩阵；MySQL Pass；SQL Server 依赖外部 Agent/nightly；**默认不切流**；见 [`cdc-kafka-pilot-2026-08-08.md`](../verification/cdc-kafka-pilot-2026-08-08.md)、[`ADR-0006`](../architecture/adr/ADR-0006-transactional-outbox-cdc-kafka-event-delivery.md)） |
