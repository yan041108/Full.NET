...
| 邮件与短信 | Notifications Providers | Provider | M5+ | Implementing（SMTP 邮件 Provider 已 **Build-verified**：MailKit 4.17.0、465/SSL 与 587/STARTTLS 模式、环境变量 Secret、条件注册和已验证收件端点；QQ SMTP 实测连接成功但认证被拒，标记 **External-auth-not-verified**，不声明邮件已发送。收件端点 API/UI、生产账号认证、容量与送达回执仍开放；短信仍为 Mapped。见[SMTP 验证](../verification/2026-09-01-notifications-smtp-provider.md)） |
