...
| Kafka Transport Capacity Runner | Build-verified | 独立 Scope A Runner 已实现低速延迟、有界开放环吞吐、不可关闭正确性门禁、可选预算、安全 checkpoint/续跑、TopicId 所有权删除保护和脱敏报告，并复用生产 Kafka 安全与性能配置构建器；公共控制面已支持稳定 ScopeCode、Driver Factory/Registry、跨 Scope 续跑隔离与临时客户端标识隔离，但默认只注册 `kafka_transport`，Scope B/C 尚未实现且只允许新增 Driver。手动专用环境工作流已提供受保护 Environment、专用 Runner、唯一 Topic、缩小 smoke/有界 matrix 与证据上传入口，但尚未实际执行。真实 Kafka 缩小测试已落盘，本机 Docker Engine 未运行且没有专用 Kafka 凭据，因此未形成新的运行证据；生产等价 Soak、N+1、故障和恢复认证仍未执行，状态继续 `Capacity-not-verified`。使用边界见[运维手册](../operations/kafka-capacity-runner.md)。 |
