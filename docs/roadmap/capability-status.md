...
| UUID v7 主键与跨库物理存储 | `Build-verified` | `UuidStorageContractV1`、008/009 双库迁移、`PrimaryKeyTypeMapping`、`validate-uuid-storage-sql`（10+ 门禁）、UUID 集成测试（Expand/Contract/Recovery 31 项）、应用持久化/外部契约测试、Runbook 与[自动化恢复演练记录](../verification/uuid-v7-primary-key-storage-2026-07-19.md)、真实栈 MySQL E2E 为 Binary16；当前声明门槛 **390/7/49/186** | 真实生产维护窗口与整库备份恢复 RPO/RTO 实跑、SQL Server 聚集索引性能基准尚未完成 |
...
| SignalR / Realtime | `Build-verified` | `IRealtimePublisher`、鉴权 Hub、用户/租户分组、MessagePack、可选 Redis Backplane，以及 Vue/Layui 认证连接、切上下文重连和通知页刷新已实现；共享/Vue/Layui 单测 **72/197/95**，Mock parity **99/99**；双库 Integration **2/2**；见[验证记录](../verification/realtime-signalr-foundation-2026-07-26.md) | 多实例 Backplane 真实栈、Outbox 修复推送与浏览器断网恢复 E2E 尚未完成 |
...
6. **P1：可靠性**——Outbox 最大重试、死信/版本共存、默认数据库租约拓扑与积压指标，Tenancy 安全关键缓存最小闭环、可信代理客户端地址与日志高优先级独立通道已达 `Build-verified`；下一优先项转向 Redis/Backplane 生产故障注入、生产指标导出/告警与日志 Task 8B 的持久 Spool/外部 Sink 演练。
7. **P1：工程门禁**——PR 集成冒烟保持 Identity/Tenancy/Outbox 核心双库 filter **8** 项，`push main` 保持完整 Integration **186** 项；全模块 `SqlDataScope`/`SqlTenantBinding`、Global Statement 精确目录与可信代理管道顺序 Architecture 门禁已落地。
