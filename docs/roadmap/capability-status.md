...
| MessagePack Outbox、租约、重试与死信 | `Build-verified` | Outbox 表、Worker、`MessageType + SchemaVersion` 精确路由、`OutboxWorker` Options（Batch/Lease/Poll/BacklogSample/MaxAttempts）、`DeadLetteredAtUtc`/`DeadLetterReasonCode` 双库迁移；`Full.NET.Outbox` 暴露无标签待处理数量与最老消息年龄；`OutboxProcessorTests` + `IntegrationEventHandlerMatcherTests` **15/15**、`OutboxRecoveryTests` + 死信迁移恢复聚焦双库 Integration **12/12**、完整 Unit **390/390**；见[Outbox Worker 运维说明](../operations/outbox-worker-topology.md)与[积压指标验证](../verification/outbox-backlog-telemetry-2026-07-26.md) | 相邻版本升级、版本退役扫描、生产多实例指标导出/告警、真实压力基准和受控人工重放自动化仍待补；未完成前不能标记为 `Verified` |
...
7. **P1：工程门禁**——PR 集成冒烟保持 Identity/Tenancy/Outbox 核心双库 filter **8** 项，`push main` 保持完整 Integration **189** 项；全模块 `SqlDataScope`/`SqlTenantBinding`、Global Statement 精确目录与可信代理管道顺序 Architecture 门禁已落地。
