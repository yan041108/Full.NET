...
| UUID v7 主键与跨库物理存储 | `Build-verified` | `UuidStorageContractV1`、008/009 双库迁移、`PrimaryKeyTypeMapping`、`validate-uuid-storage-sql`（10+ 门禁）、UUID 集成测试（Expand/Contract/Recovery 31 项）、应用持久化/外部契约测试、Runbook 与[自动化恢复演练记录](../verification/uuid-v7-primary-key-storage-2026-07-19.md)、真实栈 MySQL E2E 为 Binary16；当前声明门槛 **378/7/49/184** | 真实生产维护窗口与整库备份恢复 RPO/RTO 实跑、SQL Server 聚集索引性能基准尚未完成 |
...
| FusionCache + `.AsHybridCache()` | `Build-verified` | 单一实现、L2/Backplane、全局关闭 Fail-Safe；Tenancy 域名解析缓存采用“提交后本机修复 + Outbox/Backplane 跨节点可靠收敛”，Worker 等待 L2 删除与失效广播完成后才确认消息；`CacheConsistencyTests` 覆盖 SQL Server/MySQL 本机修复、双 API 节点 + Redis + Worker 以及 Redis 不可达，共 **6/6** 通过；见[强化模块化单体硬化验证](../verification/strengthened-modular-monolith-hardening-2026-07-26.md) | Redis/Backplane 中断后恢复、延迟 Worker、多实例指标和完整 S0/S1/S2 分级仍待补；完成前不能标记为 `Verified` |
