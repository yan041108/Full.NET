...
| Workflow | Build-verified | 已交付自有审批内核、不可变定义/表单版本、原生静态表单设计器、Workflow-Vue3 设计器适配、Vue 与 uni-app/H5 运行时、多级 `human.approval` 审批/驳回、可执行 `notify.cc`、Tenant 本地收件人候选目录、首个可执行 `gateway.exclusive`、带精确恢复权限/租户用户校验/乐观并发/幂等/审计/通知的活动待办改派，以及 Assigned/Completed/Rejected/Cancelled 事务 Outbox → Notifications Intent/Inbox 异步提醒投影；提醒按稳定 MessageId 幂等，四个缺失的内建站内信模板会在可信事件作用域内原子创建并发布，已有发布版本仍由租户管理。双库 Workflow 改派与 Outbox 断言已纳入 Actions；暂停实例强制恢复、Worker 端到端投影、Worker 恢复、逐页人工产品验收和生产容量尚未关闭，因此不得标为 `Verified`。见[活动待办改派](../verification/2026-09-05-workflow-active-todo-reassignment.md)、[异步提醒投影](../verification/2026-09-05-workflow-notifications-event-projection.md)、[Tenant 候选目录验证](../verification/2026-09-05-workflow-tenant-recipient-directory.md)、[排他网关验证](../verification/2026-09-05-workflow-exclusive-gateway-verification.md)、[`notify.cc` 验证](../verification/2026-09-04-workflow-notify-cc-verification.md)、[首切片收口验证](../verification/2026-08-31-workflow-first-slice-closeout.md)与[Native AOT 验证](../verification/2026-08-30-workflow-native-aot.md)。 |
