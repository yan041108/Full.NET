...
| MessagePack Outbox、租约、重试与死信 | `Build-verified` | Outbox 表、Worker、`MessageType + SchemaVersion` 精确路由、`OutboxWorker` Options（Batch/Lease/Poll/BacklogSample/MaxAttempts）、`DeadLetteredAtUtc`/`DeadLetterReasonCode` 双库迁移；`Full.NET.Outbox` 暴露无标签待处理数量与最老消息年龄；Worker 提供按唯一 Handler 自动覆盖 canonical/legacy 路由的一次性只读版本退役扫描，pending 或 dead-letter 任一非零都会以退出码 `2` 阻断；`OutboxProcessorTests` + `IntegrationEventHandlerMatcherTests` **15/15**、退役扫描 Unit **5/5**、`OutboxRecoveryTests` + 死信迁移恢复聚焦双库 Integration **14/14**，当前 canonical **416/7/49/191**；见[Outbox Worker 运维说明](../operations/outbox-worker-topology.md)、[积压指标验证](../verification/outbox-backlog-telemetry-2026-07-26.md)与[退役扫描验证](../verification/outbox-version-retirement-scan-2026-07-27.md) | 相邻版本升级、生产发布平台持续退役门禁、生产多实例指标导出/告警、真实压力基准和受控人工重放自动化仍待补；未完成前不能标记为 `Verified` |
...
| SignalR / Realtime | `Build-verified` | `IRealtimePublisher`、鉴权 Hub、用户/租户分组、MessagePack、可选 Redis Backplane，以及 Vue/Layui 认证连接、首次连接失败退避、切上下文重连和通知页刷新已实现；启用时 `Realtime:HubPath` 在服务注册期校验绝对路径、空白、查询与片段边界；专用 Backplane ready、环境级 Channel Prefix、后台重连和 SQL Server/MySQL 双 API 节点 Redis stop/start 恢复 **2/2** 已验证；共享/Vue/Layui 单测 **79/204/95**，Mock parity **99/99**；见[基础验证](../verification/realtime-signalr-foundation-2026-07-26.md)、[HubPath 校验](../verification/realtime-hub-path-validation-2026-07-27.md)与[故障恢复](../verification/realtime-redis-backplane-recovery-2026-07-26.md) | 生产多副本编排/告警、Redis Cluster/Sentinel、Outbox 修复推送与浏览器真实后端断网恢复 E2E 尚未完成 |
