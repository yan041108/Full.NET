...
| MessagePack Outbox、租约、重试与死信 | `Build-verified` | Outbox 表、Worker、`MessageType + SchemaVersion` 精确路由、六字段稳定消息上下文和 Handler 幂等策略启动门禁；`OutboxWorker` Options（Batch/MaxConcurrency/Lease/LeaseRenewal/Poll/BacklogSample/MaxAttempts）、`DeadLetteredAtUtc`/`DeadLetterReasonCode` 双库迁移；生产默认关闭的 `OutboxRetention` 仅小批量清理严格过期的成功终态，并支持热暂停与低基数清理指标；`Full.NET.Outbox` 以单次采样查询暴露无标签待处理数量、最老消息年龄、到期重试、活动租约、死信数量与最老死信年龄；Worker 提供按唯一 Handler 自动覆盖 canonical/legacy 路由的一次性只读版本退役扫描，pending 或 dead-letter 任一非零都会以退出码 `2` 阻断；满批次领取会立即继续排空，未满批次才进入配置轮询等待；默认 `MaxConcurrency = 1` 保持串行，显式启用时每条消息使用独立 Scope，双库已验证并发上限和成功终态；处理批次按精确消息 ID 和共享 LockId 使用独立 Scope 主动单调续租，并以单调事件顺序保证续租故障、终态与处理失败的异常优先级，双库验证慢 Handler、批尾消息与宿主时钟回拨边界；清理 off/on 的 SQL Server/MySQL 并发 1/4 真实 API 短矩阵已通过，默认批大小和单轮上限保持不变；`OutboxProcessorTests` **19/19**、Handler Matcher **5/5**、退役扫描 Unit **5/5**、Outbox retention 双库 Integration **2/2**，当前 canonical **510/7/49/199**；见[Outbox Worker 运维说明](../operations/outbox-worker-topology.md)、[消息上下文与幂等门禁验证](../verification/outbox-message-context-idempotency-2026-07-29.md)、[主动续租验证](../verification/outbox-active-lease-renewal-2026-07-28.md)、[混合负载入队基线](../verification/production-equivalent-mixed-load-2026-07-28.md)、[积压指标验证](../verification/outbox-backlog-telemetry-2026-07-26.md)、[退役扫描验证](../verification/outbox-version-retirement-scan-2026-07-27.md)与[性能硬化验证](../verification/performance-hardening-foundation-2026-07-27.md)、[Outbox 保留清理验证](../verification/outbox-retention-2026-07-29.md) | 相邻版本升级、生产发布平台持续退役门禁、生产多实例指标导出/告警、独立 Worker 消费容量基准和受控人工重放自动化仍待补；未完成前不能标记为 `Verified` |
