...
| UUID v7 主键与跨库物理存储 | `Build-verified` | `UuidStorageContractV1`、008/009 双库迁移、`MySqlConnectionStringPolicy`/`Binary16` 启动校验、UUID 集成测试（Expand/Contract/Recovery 31 项）、`GuidPrimaryKeyApplicationTests`/`GuidPrimaryKeyReadPathTests`/`UuidExternalContractIntegrationTests`、Runbook 与[自动化恢复演练记录](../verification/uuid-v7-primary-key-storage-2026-07-19.md)、真实栈 MySQL E2E 走 Binary16；门槛 **296/6/26/66** 见[测试门槛核对](test-threshold-audit-2026-07-19.md) | 真实生产维护窗口与整库备份恢复 RPO/RTO 实跑、SQL Server 聚集索引性能基准尚未完成 |
