...
| Kafka Capacity Runner | Build-verified | Scope A `kafka_transport` 与 Scope B `worker_inbox_handler` 已实现；Scope B 复用生产分区有界调度、连续 Offset 水位、Envelope、Dapper Inbox 事务、Dispatcher、Handler、Retry/DLQ 处理核心，并在 Kafka I/O 前验证专用数据库身份、schema 与 CdcKafka 所有权。SQL Server/MySQL + 真实 Kafka 缩减集成测试已通过；Scope A 手动工作流保持 A-only，Scope B 尚未在专用生产等价环境执行正式矩阵，Scope C Outbox/CDC 全链尚未实现。生产等价 Soak、N+1、故障和恢复认证仍未执行，状态继续 `Capacity-not-verified`。使用边界见[运维手册](../operations/kafka-capacity-runner.md)。 |
