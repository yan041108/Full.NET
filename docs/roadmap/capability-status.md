...
| UUID v7 主键与跨库物理存储 | `Build-verified` | `UuidStorageContractV1`、008/009 双库迁移、`PrimaryKeyTypeMapping`、`validate-uuid-storage-sql`（10+ 门禁）、UUID 集成测试（Expand/Contract/Recovery 31 项）、应用持久化/外部契约测试、Runbook 与[自动化恢复演练记录](../verification/uuid-v7-primary-key-storage-2026-07-19.md)、真实栈 MySQL E2E 为 Binary16；当前声明门槛 **392/7/49/189** | 真实生产维护窗口与整库备份恢复 RPO/RTO 实跑、SQL Server 聚集索引性能基准尚未完成 |
...
| 高并发结构化日志与 OpenTelemetry | `Build-verified` | Serilog 普通/高优先级独立有界异步队列；高优先级固定非阻塞；固定 `channel` 指标与 ready 降级检查；过载/慢 Sink 聚焦 Unit **6/6**；见[运维说明](../operations/logging-degraded-mode.md)与[验证记录](../verification/high-priority-logging-channel-2026-07-26.md) | 磁盘 Spool、外部可靠 Sink、平台不可用和进程退出有界刷新仍待 Task 8B；完成前不能标记为 `Verified` |
...
6. **P1：可靠性**——Outbox 最大重试、死信/版本共存、默认数据库租约拓扑与积压指标，Tenancy 安全关键缓存最小闭环、Realtime Redis Backplane 双节点故障恢复、可信代理客户端地址与日志高优先级独立通道已达 `Build-verified`；下一优先项转向生产编排/告警、完整 S0/S1/S2 分级和日志 Task 8B 的持久 Spool/外部 Sink 演练（见[硬化计划](../superpowers/plans/2026-07-18-architecture-hardening.md) Task 7/8）。
