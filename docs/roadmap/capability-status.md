...
| Workflow | Build-verified | 已交付自有审批内核、不可变定义/表单版本、原生静态表单设计器、Workflow-Vue3 设计器适配、Vue 与 uni-app/H5 运行时、多级线性 `human.approval` 审批/驳回，以及首个可执行 `notify.cc` 闭环（活动 Host 用户选择、同步完成步骤、实例级去重、“我的抄送”与幂等已读）；SQL Server/MySQL Integration、Host/Tenant Vue 审批矩阵和 Linux Native AOT 已有新鲜证据。Tenant 本地收件人候选目录、`gateway.exclusive`、Worker 恢复、Notifications 异步提醒投影和生产容量尚未关闭，因此不得标为 `Verified`。见[核心计划](../superpowers/plans/2026-08-20-workflow-first-vertical-slice.md)、[`notify.cc` 验证](../verification/2026-09-04-workflow-notify-cc-verification.md)、[首切片收口验证](../verification/2026-08-31-workflow-first-slice-closeout.md)与[Native AOT 验证](../verification/2026-08-30-workflow-native-aot.md)。 |
