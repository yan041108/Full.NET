...
| Jobs Host 任务定义 | `Build-verified` | Host 任务定义、手动触发、执行记录与 Worker；过期 `Running` 租约可由 SQL Server/MySQL 原子重新领取；宿主取消会向上传播且不误记业务失败；既有双库 Integration **2/2** 已纳入每库 4 Worker × 32 条执行的确定性竞争门禁，以及同批缺失 Handler/健康任务的失败隔离门禁；见[基础验证](../verification/jobs-host-definitions-2026-07-26.md)、[多 Worker 原子领取验证](../verification/jobs-multi-worker-claim-2026-07-27.md)与[取消及批次隔离验证](../verification/jobs-cancellation-batch-failure-isolation-2026-07-27.md) | Cron/延迟调度、失败重试分类、运维重放、长任务主动续租和真实大规模多 Worker 压力基准仍待补 |
