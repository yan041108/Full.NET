...
| Notifications 公告与站内信 | `Build-verified` | Host 公告、站内信、我的消息与未读数；实时推送在数据库提交后尽力执行，推送失败不反转已提交业务结果；双管理端与双库 Integration **4/4**；见[公告](../verification/notifications-host-announcement-2026-07-26.md)与[站内信验证](../verification/notifications-inbox-message-2026-07-26.md) | 管理端 SignalR 实时客户端、Outbox 修复推送与多实例真实栈仍待补 |
...
| SignalR / Realtime | `Build-verified` | `IRealtimePublisher`、鉴权 Hub、用户/租户分组、MessagePack 与可选 Redis Backplane 已实现；专用 Backplane ready、环境级 Channel Prefix、后台重连和 SQL Server/MySQL 双 API 节点 Redis stop/start 恢复 **2/2** 已验证；见[基础验证](../verification/realtime-signalr-foundation-2026-07-26.md)与[故障恢复](../verification/realtime-redis-backplane-recovery-2026-07-26.md) | 管理端实时客户端、生产多副本编排/告警、Redis Cluster/Sentinel 与 Outbox 驱动推送尚未完成 |
