...
| Workflow | Build-verified | 已交付自有审批内核、不可变定义/表单版本、原生静态表单设计器、Workflow-Vue3 设计器适配、Vue 与 uni-app/H5 运行时、多级 `human.approval` 审批/驳回、可执行 `notify.cc`，以及首个可执行 `gateway.exclusive` 闭环（发布期表单字段绑定、有序单条件分支、唯一默认分支、审批 Patch 后求值、事务内步骤/分支轨迹与拒绝短路）；SQL Server/MySQL 共享 Integration 断言已纳入 Actions 门禁。Tenant 本地收件人候选目录、Worker 恢复、Notifications 异步提醒投影、人工产品验收和生产容量尚未关闭，因此不得标为 `Verified`。见[排他网关验证](../verification/2026-09-05-workflow-exclusive-gateway-verification.md)、[`notify.cc` 验证](../verification/2026-09-04-workflow-notify-cc-verification.md)、[首切片收口验证](../verification/2026-08-31-workflow-first-slice-closeout.md)与[Native AOT 验证](../verification/2026-08-30-workflow-native-aot.md)。 |
