...
| 高并发结构化日志与 OpenTelemetry | `Build-verified` | Serilog 普通/高优先级独立有界异步队列；高优先级固定非阻塞；固定 `channel` 指标与 ready 降级检查；两条队列并行排空且共享 5 秒默认退出预算；过载、慢 Sink 与退出聚焦 Unit **9/9**；见[运维说明](../operations/logging-degraded-mode.md)、[通道验证](../verification/high-priority-logging-channel-2026-07-26.md)与[退出验证](../verification/bounded-logging-shutdown-2026-07-26.md) | 磁盘 Spool、外部可靠 Sink、平台不可用、磁盘满与跨重启重放仍待后续 Task 8B；完成前不能标记为 `Verified` |
