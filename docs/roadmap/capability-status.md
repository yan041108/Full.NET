...
| Kafka Transport Capacity Runner | Build-verified | 独立 Scope A Runner 已实现低速延迟、有界开放环吞吐、不可关闭正确性门禁、可选预算、安全 checkpoint/续跑、TopicId 所有权删除保护和脱敏报告，并复用生产 Kafka 安全与性能配置构建器；公共控制面已支持稳定 ScopeCode、Driver Factory/Registry、跨 Scope 续跑隔离与临时客户端标识隔离，但默认只注册 `kafka_transport`，Scope B/C 尚未实现且只允许新增 Driver。真实 Kafka 缩小测试已落盘，但本轮因开发机 Docker Engine 未运行未形成通过证据；专用生产等价环境认证仍未执行，状态继续 `Capacity-not-verified`。使用边界见[运维手册](../operations/kafka-capacity-runner.md)。 |
