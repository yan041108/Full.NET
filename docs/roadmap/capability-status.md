...
| Jobs Host 任务定义 | `Build-verified` | Host 任务定义、手动触发、执行记录与 Worker；过期 `Running` 租约可由 SQL Server/MySQL 原子重新领取；既有双库 Integration **2/2** 已纳入每库 4 Worker × 32 条执行的确定性竞争门禁；见[基础验证](../verification/jobs-host-definitions-2026-07-26.md)与[多 Worker 原子领取验证](../verification/jobs-multi-worker-claim-2026-07-27.md) | Cron/延迟调度、失败分类、运维重放和真实大规模多 Worker 压力基准仍待补 |
