...
| MessagePack Outbox、租约、重试与死信 | `Build-verified` | Outbox 表、Worker、`MessageType + SchemaVersion` 精确路由、`OutboxWorker` Options（Batch/MaxConcurrency/Lease/Poll/BacklogSample/MaxAttempts）、`DeadLetteredAtUtc`/`DeadLetterReasonCode` 双库迁移；`Full.NET.Outbox` 暴露无标签待处理数量与最老消息年龄；Worker 提供按唯一 Handler 自动覆盖 canonical/legacy 路由的一次性只读版本退役扫描，pending 或 dead-letter 任一非零都会以退出码 `2` 阻断；满批次领取会立即继续排空，未满批次才进入配置轮询等待；默认 `MaxConcurrency = 1` 保持串行，显式启用时每条消息使用独立 Scope，双库已验证并发上限和成功终态；`OutboxProcessorTests` **12/12**、退役扫描 Unit **5/5**、`OutboxRecoveryTests` + 死信迁移恢复聚焦双库 Integration **14/14**，当前 canonical **454/7/49/191**；见[Outbox Worker 运维说明](../operations/outbox-worker-topology.md)、[积压指标验证](../verification/outbox-backlog-telemetry-2026-07-26.md)、[退役扫描验证](../verification/outbox-version-retirement-scan-2026-07-27.md)与[性能硬化验证](../verification/performance-hardening-foundation-2026-07-27.md) | 相邻版本升级、生产发布平台持续退役门禁、生产多实例指标导出/告警、真实压力基准和受控人工重放自动化仍待补；未完成前不能标记为 `Verified` |
...
7. **P1：工程门禁**——PR 集成冒烟保持 Identity/Tenancy/Outbox 核心双库 filter **8** 项，`push main` 保持完整 Integration **191** 项；全模块 `SqlDataScope`/`SqlTenantBinding`、Global Statement 精确目录与可信代理管道顺序 Architecture 门禁已落地。
