...
| SignalR / Realtime | `Build-verified` | `IRealtimePublisher`、鉴权 Hub、用户/租户分组、MessagePack、可选 Redis Backplane，以及 Vue/Layui 认证连接、首次连接失败退避、切上下文重连和通知页刷新已实现；专用 Backplane ready、环境级 Channel Prefix、后台重连和 SQL Server/MySQL 双 API 节点 Redis stop/start 恢复 **2/2** 已验证；共享/Vue/Layui 单测 **75/200/95**，Mock parity **99/99**；见[基础验证](../verification/realtime-signalr-foundation-2026-07-26.md)与[故障恢复](../verification/realtime-redis-backplane-recovery-2026-07-26.md) | 生产多副本编排/告警、Redis Cluster/Sentinel、Outbox 修复推送与浏览器真实后端断网恢复 E2E 尚未完成 |
