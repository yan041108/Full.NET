...
| Jobs Host 任务定义 | `Build-verified` | Host 任务定义、手动触发、执行记录与 Worker；过期 `Running` 租约可由 SQL Server/MySQL 原子重新领取；宿主取消会向上传播且不误记业务失败；Worker 的批大小、轮询、初始租约与续租间隔均通过 `Jobs:Worker` 有界配置并在启动期校验；长任务按当前 `LeaseId` 主动续租，所有权丢失会取消 Handler 且不误写业务失败；Worker 满批次立即继续排空，且同批执行定义合并为一次参数化查询；双库 Integration **2/2** 已覆盖每库 4 Worker × 32 条执行竞争、批次故障隔离，以及初始租约过去后健康长任务不被重复领取；见[基础验证](../verification/jobs-host-definitions-2026-07-26.md)、[多 Worker 原子领取验证](../verification/jobs-multi-worker-claim-2026-07-27.md)、[取消及批次隔离验证](../verification/jobs-cancellation-batch-failure-isolation-2026-07-27.md)、[Worker 有界配置验证](../verification/jobs-worker-bounded-options-2026-07-27.md)与[主动续租验证](../verification/jobs-active-lease-renewal-2026-07-27.md) | Cron/延迟调度、失败重试分类、运维重放和真实大规模多 Worker 压力基准仍待补 |
