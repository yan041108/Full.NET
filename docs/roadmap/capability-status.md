...
| 最小 RBAC 与权限导航 | `Build-verified` | 当前用户、权限、Vue/Layui 导航与按钮门禁；**Host 用户**列表/创建/禁用 API + 双端 UI + 双库集成 + Mock/真实栈冒烟（见[验证记录](../verification/identity-user-management-2026-07-21.md)） | 角色/菜单/组织/数据范围 CRUD 未实现；用户资料更新 UI 与 OpenAPI 夹具待补 |
...
1. **P0：交付证明——Identity 用户管理纵向切片**——API + 双库 + Vue/Layui + 真实栈最小冒烟已交付（见[验证记录](../verification/identity-user-management-2026-07-21.md)）；下一步收敛 OpenAPI/更新 UI、再排角色/菜单/组织，禁止三刀并行。
