...
|---|---|---|---|
| 模块化单体、显式模块依赖与宿主 Profile | `Implemented` | `Full.NET.Modularity`、`Full.NET.Composition`、Api/Worker/Migrator 显式 Profile；Task 4A 已以注册快照稳定模块键、Contracts 授权入口及 30 项 Architecture Tests 关闭跨模块实现引用和生产友元 | API 仍携带迁移组件且 Migrator 仍装入完整 HTTP 服务；Task 4B～4C 与两个当前可重复但尚未由 Base 对照确定归因的 API Integration 失败未关闭前不得标记 `Build-verified` |
...
| UUID v7 主键与跨库物理存储 | `Build-verified` | `UuidStorageContractV1`、008/009 双库迁移、`PrimaryKeyTypeMapping`、`validate-uuid-storage-sql`（010+ 门禁）、UUID 集成测试（Expand/Contract/Recovery 31 项）、应用持久化/外部契约测试、Runbook 与[自动化恢复演练记录](../verification/uuid-v7-primary-key-storage-2026-07-19.md)、真实栈 MySQL E2E 走 Binary16；当前声明门槛 **341/7/30/109** | 真实生产维护窗口与整库备份恢复 RPO/RTO 实跑、SQL Server 聚集索引性能基准尚未完成 |
...
| 健康检查与编排器就绪信号 | `Implemented` | `/health/live`、`/health/ready`、`/health/startup` 路由与标签过滤已存在 | ready/startup 当前没有真实检查，空集合可返回 Healthy；完成硬化 Task 4D 的双库、Redis、startup 和故障注入前不得作为生产探针 |
...
| 模块化 Seed Baseline/Overlay | `Build-verified` | Migrator 两阶段 Seed、双库 Development/Test/Production 契约、Production 缺 Secret 拒绝；Identity 生产程序集只注册 Baseline Contributor，Task 3A 发布物扫描零命中（见[种子双库验证](../verification/seed-dual-database-contract-2026-07-21.md)、[Production Secret 验证](../verification/seed-production-secret-and-super-admin-disable-2026-07-21.md)、[运维 Runbook](../operations/seed-production-baseline.md)；门槛 **341/7/30/109**） | Task 3A 当前机器缺容器运行时，更新后的双库 Seed/真实栈未重跑；完整 Aspire/CI Profile E2E 仍开放；Production 远程超管写已可按 ADR-0004 开启，不能标记为 `Verified` |
...
4. **P1：模块和运行角色边界**——Task 4A 已移除跨模块实现引用/生产友元，并由注册快照稳定模块键和负向 Architecture Test 防回归；仍须执行 Task 4B～4C，移除 API 迁移执行能力和 Migrator 完整 HTTP 装配，并通过 Base 对照确定当前 Identity/Organization API 失败的归因后，模块 Profile 才能恢复 `Build-verified`。
5. **P1：真实健康与 Endpoint 安全意图**——ready/startup 必须检查当前数据库、已配置 Redis 与初始化状态，空集合不得作为成功证据；所有 Endpoint 显式认证或匿名（硬化 Task 4D～4E）。
6. **P1：可靠性**——Outbox 最大重试/死信/版本共存、**多 Worker 租约压力与部署拓扑文档**、TenantRequired/Global SQL 语义门禁、缓存一致性分级和高优先级日志通道（见[硬化计划](../superpowers/plans/2026-07-18-architecture-hardening.md) Task 6 扩展）。
7. **P1：工程门禁**——PR 集成冒烟从“仅迁移 2 项”加宽到 Identity/Tenancy/Outbox 核心 filter（目标 ≤15m）；门槛审计与 CI **341/7/30/109** 保持同步；Architecture Tests 随模块增长补表所有权与 SqlDataScope 显式性。
...
11. **P2：运维与可维护性**——JWT 轮换 / Outbox 死信 / Redis 故障 / Seed 失败 Runbook；按行为不变方式拆分 `IdentityModule.AddServices`；Login Handler 只在相邻行为变更或基准支持时拆分。
