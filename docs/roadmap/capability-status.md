...
| UUID v7 主键与跨库物理存储 | `Build-verified` | `UuidStorageContractV1`、008/009 双库迁移、`PrimaryKeyTypeMapping`、`validate-uuid-storage-sql`（10+ 门禁）、UUID 集成测试（Expand/Contract/Recovery 31 项）、应用持久化/外部契约测试、Runbook 与[自动化恢复演练记录](../verification/uuid-v7-primary-key-storage-2026-07-19.md)、真实栈 MySQL E2E 为 Binary16；当前声明门槛 **386/7/49/184** | 真实生产维护窗口与整库备份恢复 RPO/RTO 实跑、SQL Server 聚集索引性能基准尚未完成 |
...
| Identity Host API Key | `Build-verified` | Host API Key 创建/列表/禁用与认证；认证 SQL 可在主体建立前安全读取 Host 目录；创建权限同时受操作者有效权限与目标用户权限快照上限约束；双库 Integration **2/2**；见[验证记录](../verification/identity-api-key-2026-07-26.md) | 仅 Host 作用域；无双管理端管理 UI；轮换与使用审计仍待补 |
...
6. **P1：可靠性**——Outbox 最大重试、死信/版本共存、默认数据库租约拓扑、Tenancy 安全关键缓存最小闭环、可信代理客户端地址与日志高优先级独立通道已达 `Build-verified`；下一优先项转向 Redis/Backplane 生产故障注入与日志 Task 8B 的持久 Spool/外部 Sink 演练。
7. **P1：工程门禁**——PR 集成冒烟保持 Identity/Tenancy/Outbox 核心双库 filter **8** 项，`push main` 保持完整 Integration **184** 项；全模块 `SqlDataScope`/`SqlTenantBinding`、Global Statement 精确目录与可信代理管道顺序 Architecture 门禁已落地。
