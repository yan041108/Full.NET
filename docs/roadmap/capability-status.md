...
| Files Host 文件元数据 | `Build-verified` | `027_FilesFile.sql`；上传/列表/下载/软删除；本地存储 Provider；软删除提交后再尽力删除 Blob；`Files:Local` 启动期校验；双端 UI；Integration **2/2**；Mock parity **2/2**；真实栈 `host-files` SQL Server **4/4** + MySQL **4/4**；见[验证记录](../verification/files-host-file-metadata-2026-07-26.md) | S3/OSS Provider、租户文件与孤立 Blob 自动清理；全量真实栈矩阵依赖 CI；不能标记为 `Verified` |
| Identity Host API Key | `Build-verified` | Host API Key 创建/列表/禁用与认证；认证 SQL 可在主体建立前安全读取 Host 目录；创建权限同时受操作者有效权限与目标用户权限快照上限约束；Vue/Layui 双管理端完成列表、创建、一次性明文复制与禁用，Mock API Playwright **2/2**；双库 Integration **2/2**；见[验证记录](../verification/identity-api-key-2026-07-26.md) | 仅 Host 作用域；真实后端浏览器链路、轮换与使用审计仍待补 |
| Notifications 公告与站内信 | `Build-verified` | Host 公告、站内信、我的消息与未读数；实时推送在数据库提交后尽力执行，推送失败不反转已提交业务结果；Vue/Layui 已接入认证 SignalR 连接、首次失败退避恢复、切租户重连、真实未读徽标和当前通知页刷新；双管理端与双库 Integration **4/4**；见[公告](../verification/notifications-host-announcement-2026-07-26.md)、[站内信](../verification/notifications-inbox-message-2026-07-26.md)与[Realtime 验证](../verification/realtime-signalr-foundation-2026-07-26.md) | Outbox 修复推送、多实例真实栈与浏览器断网恢复 E2E 仍待补 |
| Jobs Host 任务定义 | `Build-verified` | Host 任务定义、手动触发、执行记录与 Worker；过期 `Running` 租约可由 SQL Server/MySQL 原子重新领取；宿主取消会向上传播且不误记业务失败；Worker 的批大小、轮询、初始租约、续租间隔与最大并发均通过 `Jobs:Worker` 有界配置并在启动期校验；`MaxConcurrency` 默认保持 `1`，显式启用时不同 JobKey 有界并行、相同 JobKey 保持串行且每条执行使用独立 Scope；长任务按当前 `LeaseId` 主动续租，所有权丢失会取消 Handler 且不误写业务失败；Worker 满批次立即继续排空，且同批执行定义合并为一次参数化查询；双库默认并发 smoke **2/2** 通过，并发路径 Unit 已验证峰值、同键顺序与 Scope 隔离；见[基础验证](../verification/jobs-host-definitions-2026-07-26.md)、[多 Worker 原子领取验证](../verification/jobs-multi-worker-claim-2026-07-27.md)、[取消及批次隔离验证](../verification/jobs-cancellation-batch-failure-isolation-2026-07-27.md)、[Worker 有界配置验证](../verification/jobs-worker-bounded-options-2026-07-27.md)、[主动续租验证](../verification/jobs-active-lease-renewal-2026-07-27.md)与[有界并发验证](../verification/jobs-bounded-concurrency-2026-07-29.md) | 并发路径的双库慢 Handler、续租、故障隔离与容量 A/B 尚未完成；未取得双库收益证据前默认并发保持 `1`；Cron/延迟调度、失败重试分类和运维重放仍待补 |
...
| SignalR / Realtime | `Build-verified` | `IRealtimePublisher`、鉴权 Hub、用户/租户分组、MessagePack、可选 Redis Backplane，以及 Vue/Layui 认证连接、首次连接失败退避、切上下文重连和通知页刷新已实现；启用时 `Realtime:HubPath` 在服务注册期校验绝对路径、空白、查询与片段边界；专用 Backplane ready、环境级 Channel Prefix、后台重连和 SQL Server/MySQL 双 API 节点 Redis stop/start 恢复 **2/2** 已验证；共享/Vue/Layui 单测 **79/206/102**，Mock parity **99/99**；见[基础验证](../verification/realtime-signalr-foundation-2026-07-26.md)、[HubPath 校验](../verification/realtime-hub-path-validation-2026-07-27.md)与[故障恢复](../verification/realtime-redis-backplane-recovery-2026-07-26.md) | 生产多副本编排/告警、Redis Cluster/Sentinel、Outbox 修复推送与浏览器真实后端断网恢复 E2E 尚未完成 |
