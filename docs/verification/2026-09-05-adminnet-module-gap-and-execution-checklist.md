...
| DC02 | 版本回滚/删除 | DocumentVersion/Rollback、DeleteVersion | Host 已提供版本删除、保留策略读写（迁移 232）与 Worker 选项；显式「回滚为当前版本」指针操作仍按 DC01 验收项跟踪，不与保留清理混为一谈 | 部分；47、62 |
| DC03 | 文档附加功能 | 热门/推荐标签、名称冲突、文档访问统计/日志、批量分享 | 热门/推荐标签、同分类标题唯一、访问日志/统计、批量分享与分享列表筛选已落地（2026-09 二期） | 已有；63 统计页签可继续扩展指标 |
