...
- 状态：**Build-verified**（双库 Integration、迁移 055 恢复、Vue/Layui 逐按钮门控、真实栈 E2E）
...
| Layui 权限门 | `index.html` + `roles.js` `data-permission` + Vitest |
...
| `pnpm --filter @fullnet/admin-layui test -- roles.test.js` | 见交付提交 |
...
Identity Roles 作为 W1 首个完整子切片已满足：角色可授予页面/单操作、Vue 与 Layui 无权限不渲染、直调 API 返回 `authorization.permission_denied`、迁移 055 双库可恢复。库存见 [`admin-action-permission-inventory.md`](../roadmap/admin-action-permission-inventory.md)。