...
3. 创建 4 个独立依赖注入作用域，每个作用域解析自己的
   `JobExecutionRunner`；
4. 通过同一个异步起跑信号同时释放 4 个 Worker，每个 Worker 以批大小 8
   有界重复轮询，直到一次领取不足一批；这与生产 Worker 在 `READPAST` 暂时跳过
   其他 Worker 锁定行后继续下一轮的语义一致；
5. 断言 4 个 Worker 的处理总数恰好为 32；
...
后续完整 Integration 在高并发调度下出现一次 SQL Server 单轮处理总数
**24/32**：`READPAST` 允许某个 Worker 在其他查询持锁时暂时看不到候选行，而旧夹具
只轮询一次并错误要求每个 Worker 恰好领取 8 条。失败用例在相同代码下聚焦复跑
**1/1** 通过，确认这是调度相关的夹具假设。夹具现改为每个 Worker 按批大小 8
有界重复轮询；总数 32、`AttemptCount = 1` 与最终租约清理断言保持不变，生产代码与
领取 SQL 仍未修改。
