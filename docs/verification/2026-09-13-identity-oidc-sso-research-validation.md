...
| 真实栈 E2E | Playwright `vue-admin-oidc-center`（25175，`admin-oidc-center.spec.mjs`：登录、刷新、Host／租户工作台 `/api/v1/me` 探针、工作流待办页与同意／驳回操作（§6 审批探针）、Agent 工具页（§6 V12/V21 UI 探针）、后台任务定义页与触发操作（§6 后台任务探针）、切租户并返回 Host、租户内受保护页面、退出后受保护路由回登录／refresh 拒绝、强制下线后无法用已撤销 refresh 恢复（§6 在线会话探针））；`vue-admin`（25173）`auth-smoke` 断言 legacy 不展示身份中心入口 |
