...
| 项 | 证据 |
| --- | --- |
| 基线 | 分支 `main`；记录 HEAD `a96f8072`（切片 157 前）；默认 `legacy` 用户名密码登录不变 |
...
| 聚焦验证（Windows 本地，2026-09-16） | `ui/admin` OIDC 相关 Vitest **35/35**；`@fullnet/client-contracts` `identity-auth-config` + `oidc-interactive-auth` **11/11**；`admin-real-stack` `spec-contracts` 治理测试登记 oidc-center 项目与 §6 探针契约；E2E 聚焦入口 `pnpm test:e2e:real:oidc-center`（**46** 用例，待 CI／本机真实栈执行） |
...
| 上下文切换 | 切租户后工作流／Agent 工具／后台任务／Agent Run API 与 Agent 运行页 UI 创建；切租户并返回 Host 后四类 API 往返（含 Agent Run）；切租户后创建的 Agent Run 在退出／强撤后绑定失效 |
