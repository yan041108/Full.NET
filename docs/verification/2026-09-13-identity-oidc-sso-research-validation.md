...
| 项 | 证据 |
| --- | --- |
| 基线 | 分支 `main`；记录 HEAD `8a44fdb9`（切片 165 前）；默认 `legacy` 用户名密码登录不变 |
...
| 聚焦验证（Windows 本地，2026-09-16） | `ui/admin` OIDC 相关 Vitest **35/35**；`@fullnet/client-contracts` `identity-auth-config` + `oidc-interactive-auth` **11/11**；`admin-real-stack` `spec-contracts` 治理测试登记 oidc-center 项目与 §6 探针契约；E2E 聚焦入口 `pnpm test:e2e:real:oidc-center`（**54** 用例，待 CI／本机真实栈执行） |
...
| 上下文切换 | 切租户后工作流／Agent 工具／后台任务／Agent Run API 与 Agent 运行页打开／UI 创建／加载／取消；切租户并返回 Host 后四类 API 往返与 Agent 运行页打开／UI 创建／加载；切租户后创建的 Agent Run 在退出／强撤后绑定失效 |
| 工具／审批／后台任务 | UI 页面与操作（同意／驳回、任务触发、Agent 工具／Agent 运行页 Host／租户／切租户返回 Host 打开、UI 创建与 Host／租户／切租户返回 Host 加载／Host／租户取消运行）；工作流／Agent 工具／后台任务定义与执行历史 API 正／负探针；OIDC 创建并读取排队 Agent Run 正探针；退出／强撤后触发、列举定义与执行历史绑定失效；已排队 Agent Run 读取／取消／恢复／新建绑定失效（`expectRevokedOidcCenterAgentRunAccessRejected`） |
