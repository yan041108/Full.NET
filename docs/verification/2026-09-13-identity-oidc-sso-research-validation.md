...
| 项 | 证据 |
| --- | --- |
| 基线 | 分支 `main`；记录 HEAD `d722267f`（切片 141 前）；默认 `legacy` 用户名密码登录不变 |
...
| 聚焦验证（Windows 本地，2026-09-16） | `ui/admin` OIDC 相关 Vitest **35/35**；`@fullnet/client-contracts` `identity-auth-config` + `oidc-interactive-auth` **11/11**；`admin-real-stack` `spec-contracts` 治理测试登记 oidc-center 项目与 §6 探针契约；E2E 聚焦入口 `pnpm test:e2e:real:oidc-center`（**32** 用例，待 CI／本机真实栈执行） |
...
| 入口组 | 已编写探针 |
| --- | --- |
| 在线会话管理 | 应用退出／强制下线后 access／refresh 拒绝、受保护路由回登录、已撤销 refresh 无法恢复；强撤独立 API 负探针与 `revokeCurrentOidcCenterSession` 辅助 |
