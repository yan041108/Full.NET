...
| 项 | 证据 |
| --- | --- |
| 基线 | 分支 `main`；记录 HEAD `e6404195`（切片 148 前）；默认 `legacy` 用户名密码登录不变 |
...
| 聚焦验证（Windows 本地，2026-09-16） | `ui/admin` OIDC 相关 Vitest **35/35**；`@fullnet/client-contracts` `identity-auth-config` + `oidc-interactive-auth` **11/11**；`admin-real-stack` `spec-contracts` 治理测试登记 oidc-center 项目与 §6 探针契约；E2E 聚焦入口 `pnpm test:e2e:real:oidc-center`（**36** 用例，待 CI／本机真实栈执行） |
