...
| 项 | 证据 |
| --- | --- |
| 基线 | 分支 `main`；记录 HEAD `92e3be35`（切片 150 前）；默认 `legacy` 用户名密码登录不变 |
...
| 工具／审批／后台任务 | UI 页面与操作（同意／驳回、任务触发）；工作流／Agent 工具／后台任务定义与执行历史 API 正／负探针；OIDC 创建并读取排队 Agent Run 正探针；退出／强撤后触发、列举定义与执行历史绑定失效；已排队 Agent Run 读取／取消／恢复绑定失效 |
...
| 已排队 Agent Run 读取／取消／恢复 API | 独立负探针 | 独立负探针 |
