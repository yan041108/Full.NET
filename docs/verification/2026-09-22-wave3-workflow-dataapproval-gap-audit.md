...
清单 01–12 **产品代码已基本到位**；DataApproval 请求在 API 主机重启后的恢复已有 SQL Server/MySQL 集成证据。Wave 3 剩余工作是 DataApproval 应用冲突 real-stack、待办历史有数据时的详情断言、Workflow 表单附件 real-stack，以及 Workflow Recovery 专属触发/恢复 full E2E。通知投影、Workflow 实例列表与业务单据跳转已有 fresh CI 证据；不应对 05–09 重复立项。parity `Verified` 仍受 [B2 运行手册](2026-09-22-b2-verified-gates-runbook.md) 约束。
