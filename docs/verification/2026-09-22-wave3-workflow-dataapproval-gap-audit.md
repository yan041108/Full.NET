...
|----|----------|----------|----------|----------|
| 01 | 已实现 | `DataApprovalScenarioService`、`DataApprovalScenariosView.vue` | OpenAPI `data-approvals-contract.test.mjs`；本切片补 `DataApprovalApiAssertions` 双库 | real-stack 场景页 E2E |
| 02 | 已实现 | 流水号强类型提案、`SerialNumberRulesView` 提交审批 | Unit `DataApprovalRequestApplicationServiceTests` | real-stack 审批提交流 E2E |
...
| 06 | 已实现 | `workflowListMyTodoHistory`、`WorkflowTodosView` history | 同上 | 已办分页 real-stack |
...
| 12 | 已实现 | `subtable` 组件目录、Designer/Renderer | `WorkflowFormDesigner.test.ts` | 子表行界 real-stack |
| — | 部分 | Recovery Worker、通知投影 | `WorkflowRecoveryWorkerTests`、验证记录 2026-09-05 | **Linux Worker 原生 + 通知投影 E2E**（环境门禁，本波登记未跑） |
