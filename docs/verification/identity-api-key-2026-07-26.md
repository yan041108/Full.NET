...
| 管理 API | `ManageHostApiKeys` 创建/列表/禁用/轮换 |
| OpenAPI | `identity-host-api-keys-v1.json` + Scalar `ApiKey` scheme；离线契约 **2/2** |
| client-contracts | `host-api-keys.ts` + Vitest **69/69** |
| 双管理端 | Vue/Layui 列表、创建、轮换、一次性明文复制与禁用；`identity.api_keys.read/write` 分权；列表展示格式化 `lastUsedAtUtc` 与「刷新列表」 |
| 浏览器等价 | Mock API Playwright **2/2**（Vue + Layui），覆盖权限去重、轮换、明文不进入 Web Storage 与禁用 |
| 真实栈浏览器 | `host-api-keys.spec.mjs` **6/6**（Vue 3 + Layui 3）：列表、创建→ApiKey 认证后刷新列表见最后使用→轮换→禁用、受限账号 403 |
...
- 真实栈 API Key E2E（`FULLNET_E2E_SKIP_BOOTSTRAP=1` + `http://localhost:5149`）：**6/6**
- 全客户端生产构建、治理、Skill、工作区与依赖审计门禁：通过

## 已知限制

- 仅 Host 作用域；Mock 与真实栈浏览器均已覆盖轮换
- API Key 权限在创建时固定，并同时受操作者当前有效权限与目标用户权限快照上限约束
- `/api/v1/me` 等依赖会话 Claim 的端点不支持 ApiKey 主体
- API Key 轮换：`POST /api/v1/identity/api-keys/{id}/rotate` 单事务禁用旧钥并签发新钥；双库 Integration 续跑通过
- 使用审计：认证成功后后端节流更新 `LastUsedAtUtc`（最多 5 分钟一次）；双管理端列表展示格式化「最后使用」并提供「刷新列表」；真实栈 E2E 断言认证后刷新可见非「从未」
