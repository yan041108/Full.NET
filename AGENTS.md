...
1. 必须执行与风险相称的构建、测试和静态检查，并依据新鲜输出报告结果。服务端相关任务先运行 `pnpm test:integration:affected:plan -- --base <任务基线>` 审查影响集，再运行 `pnpm test:integration:affected -- --base <任务基线>`；本地任务只运行受影响测试，禁止运行 195 项全量。完整 195 项只保留给 `main` CI 的互斥并行分片门禁。
