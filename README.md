...
dotnet tests/Full.NET.UnitTests/bin/Release/net10.0/Full.NET.UnitTests.dll --minimum-expected-tests 246
...
当前本地真实数据仍由 Migrator 的 `--seed-local` 硬编码入口创建。模块化种子管道的 S0 已实现封闭 Profile、CLI 兼容解析、Contributor 契约验证和确定性依赖排序；数据库运行锁、执行审计、真实 Baseline/Overlay Contributor 与双库 E2E 仍属于 S1-S2，因此 Production 尚不能使用新管线播种。设计与后续步骤见[种子数据模块设计](docs/superpowers/specs/2026-07-17-seed-data-module-design.md)和[实施计划](docs/superpowers/plans/2026-07-17-seed-data-module.md)。
