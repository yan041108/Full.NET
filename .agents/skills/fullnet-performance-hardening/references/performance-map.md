...
dotnet tests/Full.NET.UnitTests/bin/Release/net10.0/Full.NET.UnitTests.dll --no-ansi --progress off --minimum-expected-tests 475
...
测试数量变化时先同步 canonical 门槛，再使用新数字。数据库、认证、共享宿主、Outbox、缓存或 Dapper 基础设施变更按照 `rules/development-quality.md` 第 11.1 节决定是否运行 `pnpm test:integration:full`。
